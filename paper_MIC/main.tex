...
We present an adaptive operator selection (AOS) layer for NSGA-II that treats each (crossover, mutation) pipeline as a bandit arm and selects one arm per generation, updating an online policy using survival and diversity-based rewards (optionally including a hypervolume-improvement proxy).
On \AOSNProblems benchmark problems from ZDT/DTLZ/WFG with \AOSNSeeds seeds at a fixed evaluation budget, AOS improves the median normalized hypervolume from \AOSHVMedianBaseline{} to \AOSHVMedianAOS{} with a median runtime overhead of \AOSRuntimeOverheadPct{}\%.
\keywords{Adaptive operator selection \and Multi-armed bandits \and NSGA-II \and Multiobjective optimization}
\end{abstract}
