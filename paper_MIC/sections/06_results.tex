...
At 10{,}000 evaluations (10\% of budget), AOS achieves a mean HV of \AOSHVAOSTenK{} vs.\ the baseline's \AOSHVBaselineTenK{}---a \AOSConvergenceAdvPct{}\% advantage.
...
The gap narrows toward the full budget as all methods approach the reference front, but AOS maintains a +2.3\% advantage at the final 100{,}000 evaluation mark.
...
\caption{Mean normalized HV across all 21 problems vs.\ evaluations. AOS converges significantly faster than both the baseline and random arm selection, with the largest advantage at early checkpoints and a sustained +2.3\% lead at 100K evaluations.}
...
\caption{Mean$\pm$std normalized hypervolume vs.\ evaluations for selected problems. AOS converges substantially faster on DTLZ6 and DTLZ7, and maintains or extends the lead through the full budget on WFG1 and WFG2.}
\label{fig:anytime}
\end{figure}
...
AOS achieves its strongest gains on DTLZ problems (+7.4\% family average), where landscape features (multi-modality, degenerate fronts, disconnected regions) strongly reward operator adaptation.
On WFG, AOS shows a mixed result: significant wins on WFG1--2 and WFG6, but losses on WFG4--5, WFG7--8, where concave fronts and SBX with $\eta{=}20$ are already near-optimal.
