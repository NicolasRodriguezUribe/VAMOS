...
We first compare the three methods at the benchmark-family level (UF, LSMOP, C-DTLZ, DC-DTLZ, MW), then inspect convergence speed, per-problem deltas, and operator usage.
...
AOS outperforms the baseline on \AOSWins{} of \AOSNProblems{} problems across all five families:
\begin{itemize}
  \item \textbf{UF}: +9\% HV, winning on 8 of 10 problems (including UF9, where the baseline previously dominated).
  \item \textbf{LSMOP}: +56\% HV, producing non-zero HV on problems where the baseline completely fails (e.g., LSMOP8: AOS 0.252 vs.\ baseline 0.000).
  \item \textbf{C-DTLZ}: modest gains (+0.5\%), as the baseline is already well-suited to these constrained DTLZ problems.
  \item \textbf{DC-DTLZ}: +37\% HV, with dc1dtlz3 showing +376\% improvement---the baseline nearly fails on this discontinuously constrained landscape.
  \item \textbf{MW}: +15\% HV, with mw5 exhibiting +73\% gain where the baseline achieves only half the Pareto front.
\end{itemize}
The Wilcoxon tests (Table~\ref{tab:stat_test}) confirm \StatWinsBase{} significant wins and \StatLossesBase{} significant losses against the baseline after Holm--Bonferroni correction across all \AOSNProblems{} problems.
...
The AOS advantage grows monotonically over time: from +\AOSConvergenceAdvPct{}\% at 10{,}000 evaluations to +17\% at the full 100{,}000-evaluation budget.
...
After generation 300 ($\approx$30K evaluations), arm elimination activates, pruning underperforming operators on problems where a single arm dominates. This stabilizes performance on tri-objective UF problems and well-suited constrained problems while preserving the portfolio's diversity advantage on LSMOP and challenging constrained landscapes.
...
\caption{Mean normalized HV across all \AOSNProblems{} problems (UF, LSMOP, C-DTLZ, DC-DTLZ, MW) vs.\ evaluations. AOS and the random arm both substantially outperform the fixed baseline, with the advantage growing over time rather than diminishing.}
...
\caption{Mean$\pm$std normalized HV vs.\ evaluations for selected problems. UF1 and UF7 show AOS converging rapidly to near-optimal HV; LSMOP1 and LSMOP4 demonstrate the portfolio's advantage in high dimensions; MW5 and DC1-DTLZ3 illustrate massive gains on constrained landscapes.}
\label{fig:anytime}
\end{figure}
...
\caption{Per-problem mean HV change (AOS $-$ Baseline), sorted by $\Delta$. Positive bars indicate problems where AOS outperforms the fixed operator. The large positive bars on LSMOP, constrained, and UF problems demonstrate AOS's strength across all three challenge axes.}
\label{fig:hv_delta}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{Constrained problems}: AOS wins on 11 of 13 constrained problems. The gains are particularly dramatic on problems where feasibility boundaries interact with search geometry: dc1dtlz3 (+376\%, where the baseline nearly fails to find the feasible Pareto front) and mw5 (+73\%, where the baseline covers only half the front). The two small losses (c1dtlz3 at $-$1.9\%, dc2dtlz3 at $-$9.7\%) are both statistically insignificant ($p > 0.45$).
\end{itemize}
...
AOS achieves \StatWinsBase{} significant wins vs.\ the baseline with \emph{zero} significant losses across all \AOSNProblems{} problems---including unconstrained, large-scale, and constrained benchmarks---confirming that the gains are robust across problem types and that AOS never significantly degrades performance compared to the fixed operator.
Against the random arm, AOS achieves \StatWinsRand{} significant wins with \StatLossesRand{} significant losses; the losses are concentrated on constrained problems where portfolio diversity alone is the dominant factor (Section~\ref{sec:discussion}).
This trade-off is deliberate: AOS sacrifices some peak diversity benefit in exchange for robustness guarantees against the baseline.
...
The overhead is moderate and consistent across families: it is proportionally larger on the constrained problems (12--15 variables, very cheap evaluations) and smaller on LSMOP (100 variables), reflecting the fixed per-generation cost of the bandit policy relative to the variable cost of function evaluation.
