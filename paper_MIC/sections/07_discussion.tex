...
The results demonstrate that AOS with arm elimination provides robust performance gains on challenging landscapes, winning or tying against the fixed baseline on all \AOSNProblems{} problems after statistical correction.
...
Across the full benchmark suite, the number of statistically significant losses drops to zero.
...
This is visible in the AOS-vs-random comparison, where AOS significantly loses on \StatLossesRand{} problems---all cases where elimination removed arms that would have contributed positively.
However, from a practical perspective, the relevant comparison is AOS vs.\ baseline (``should I use AOS or a fixed operator?''), and here the result is unambiguous: \StatWinsBase{} significant wins, zero significant losses.

The random arm, while achieving higher mean HV, suffers from unreliable performance on problems where the baseline excels (its losses on UF8--UF10 are far larger than AOS's near-ties).
...
\begin{enumerate}
  \item It never significantly degrades performance compared to the fixed baseline, even on problems where the baseline operator is well-suited.
  \item It provides substantial gains (+18\% mean HV) on problems where operator choice matters.
  \item The arm elimination mechanism requires no problem-specific tuning---the same $z$-threshold and warm-up period work across all 19 problems tested.
\end{enumerate}
