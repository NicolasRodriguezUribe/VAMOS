...
We evaluate on \AOSNProblems{} continuous multiobjective benchmarks drawn from three challenge axes specifically chosen to stress-test operator choice:
\begin{itemize}
  ...
  \item \textbf{Constrained problems}: 13 problems from three constrained families---C-DTLZ~\cite{jain2014cdtlz} (3 problems, 12 variables), DC-DTLZ~\cite{li2019dcdtlz} (4 problems, 12 variables), and MW~\cite{ma2019mw} (6 problems, 15 variables)---all with 2 objectives.
        Feasibility boundaries create additional operator-choice pressure: operators must simultaneously navigate toward the Pareto front and maintain feasibility, and the most effective operator can change as the population transitions from infeasible to feasible regions.
\end{itemize}
Together these \AOSNProblems{} problems cover three axes where standard SBX-based NSGA-II is known to struggle---curved Pareto sets, high dimensionality, and constrained feasible regions---making them an ideal testbed for evaluating whether adaptive operator selection provides genuine benefit beyond what a well-tuned fixed operator can achieve.
...
Hypervolume is computed using reference fronts: analytically derived true Pareto fronts for UF and LSMOP, and empirically generated approximations (50 seeds $\times$ 200{,}000 evaluations, merged and Pareto-filtered) for the constrained families.
...
Problems & \AOSNProblems{} (UF + LSMOP + C-DTLZ + DC-DTLZ + MW) \\
