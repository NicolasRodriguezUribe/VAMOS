...
1) Implement the algorithm core under `src/vamos/algorithm/`:
...
2) Add a config dataclass/builder to `src/vamos/algorithm/config.py` if needed so configs are serializable and validated.

3) Register the algorithm in `src/vamos/algorithm/registry.py`:
...
4) (Optional) Wire it into the factory (`src/vamos/algorithm/factory.py`) if you want it accessible via runner/CLI presets.
