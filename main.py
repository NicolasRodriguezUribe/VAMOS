...
from algorithm.config import MOEADConfig, NSGAIIConfig, SMSEMOAConfig
from algorithm.moead import MOEAD
from algorithm.nsgaii import NSGAII
from algorithm.smsemoa import SMSEMOA
...
def _resolve_kernel(engine_name: str):
    ...
    if engine_name == "moocore":
        try:
            from kernel.moocore_backend import MooCoreKernel
        except ImportError as exc:
            raise SystemExit(
    ...
    if engine_name == "moocore_v2":
        try:
            from kernel.moocore_backend import MooCoreKernelV2
        except ImportError as exc:
            raise SystemExit(
