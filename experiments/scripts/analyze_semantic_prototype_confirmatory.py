...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze a semantic-prototype-SBX confirmatory pilot output.")
    parser.add_argument("input_dir", type=Path, help="Pilot output directory to analyze.")
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Output directory for confirmatory artifacts. Defaults to the input directory.",
    )
    ...
    confirmatory_summary_rows = build_confirmatory_summary(case_rows)
    ...
    confirmatory_report = build_confirmatory_report(
        config=config,
    ...
    memo_text = render_confirmatory_findings_memo(
        config=config,
        confirmatory_summary_rows=confirmatory_summary_rows,
        prototype_profile=prototype_profile,
        phase_diagnostics=phase_diagnostics,
    ...
    write_confirmatory_outputs(
        ...
        confirmatory_summary_rows=confirmatory_summary_rows,
        confirmatory_report=confirmatory_report,
        memo_text=memo_text,
        final_verdict=final_verdict,
    )
    print(f"[confirmatory-analysis] wrote {output_dir / 'confirmatory_summary.csv'}")
    print(f"[confirmatory-analysis] wrote {output_dir / 'confirmatory_report.json'}")
    print(f"[confirmatory-analysis] wrote {output_dir / 'confirmatory_findings_memo.md'}")
    print(f"[confirmatory-analysis] wrote {output_dir / 'confirmatory_final_verdict.json'}")
    print(f"[confirmatory-analysis] verdict={final_verdict['verdict']}")
