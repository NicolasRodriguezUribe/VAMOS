...
def _make_subtable(section_name: str, summary: pd.DataFrame) -> str:
    """Generate a single subtable (no caption/label) for side-by-side layout."""
    lines = [
        r"\begin{tabular}{lcccc}",
        r"\toprule",
        rf"\multicolumn{{5}}{{c}}{{\textbf{{{section_name}}}}} \\",
        r"\midrule",
...
def make_merged_latex_table(sections: list[tuple[str, pd.DataFrame]], *, label: str, caption: str) -> str:
    """Generate a two-column side-by-side table with one subtable per algorithm variant."""
    lines = [
    ...
    subtables = [_make_subtable(name, summary) for name, summary in sections]
    lines.append(r"\begin{minipage}{0.48\textwidth}\centering")
    lines.append(subtables[0])
    lines.append(r"\end{minipage}\hfill")
    lines.append(r"\begin{minipage}{0.48\textwidth}\centering")
    lines.append(subtables[1])
