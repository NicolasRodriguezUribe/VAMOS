...
\subsection{Adaptive arm elimination}
While a diverse portfolio is generally beneficial, it can be counterproductive on problems where the default operator is already well-suited---each pull of a suboptimal arm wastes part of the evaluation budget.
To address this, we introduce an \emph{arm elimination} mechanism that prunes statistically underperforming operators after an initial learning phase.

After a configurable warm-up of $T_{\mathrm{elim}}$ generations, the controller computes the mean reward $\bar{r}_k$ and standard deviation $s_k$ for each active arm $k$ from its accumulated reward history.
For each arm $k \neq k^*$ (where $k^* = \arg\max_k \bar{r}_k$), we compute the $z$-score of the difference from the best arm:
\begin{align}
  z_k = \frac{\bar{r}_{k^*} - \bar{r}_k}{\sqrt{s_{k^*}^2/n_{k^*} + s_k^2/n_k}},
\end{align}
where $n_k$ is the number of reward observations for arm $k$.
Arms with $z_k \geq z_{\mathrm{thresh}}$ are permanently eliminated: they are excluded from all subsequent selections, including the exploration floor and warm-up draws.
A safety constraint ensures that at least $K_{\min}$ arms remain active.

This mechanism is specifically designed to handle the robustness--diversity trade-off: on problems where multiple operators contribute (e.g., high-dimensional LSMOP), reward differences are small and few or no arms are eliminated; on problems where a single operator dominates (e.g., tri-objective UF with spherical fronts), weaker arms are pruned early, concentrating the budget on productive operators.

\begin{algorithm}[t]
\DontPrintSemicolon
\small
\KwIn{Portfolio $\mathcal{A}=\{1,\dots,K\}$; policy $\pi$; pop.\ size $N$; weights $w_{\mathrm{surv}}, w_{\mathrm{nd}}, w_{\mathrm{hv}}$; floor $p_{\mathrm{floor}}$; warmup $m_{\min}$; budget $B$}
\KwOut{Final population $P$}
$P_0 \gets \mathtt{InitPopulation}(N)$\; \label{alg:aos:init}
$c_k \gets 0$ \textbf{for all} $k \in \mathcal{A}$\; \label{alg:aos:counts}
\For{$t = 1$ \KwTo $\lfloor B/N \rfloor$}{ \label{alg:aos:for}
    \tcc{Arm selection (warmup $\to$ floor $\to$ policy)}
    \uIf{$\exists\, k \in \mathcal{A}$ \textbf{with} $c_k < m_{\min}$}{ \label{alg:aos:warmup}
        $a_t \gets \arg\min_k c_k$\;
    }
    \uElseIf{$\mathtt{Uniform}(0,1) < p_{\mathrm{floor}}$}{ \label{alg:aos:floor}
        $a_t \gets \mathtt{Uniform}(\mathcal{A})$\;
    }
    \Else{ \label{alg:aos:policy}
        $a_t \gets \pi.\mathtt{select}()$\;
    }
    $c_{a_t} \gets c_{a_t} + 1$\; \label{alg:aos:count}
    \tcc{Variation and survival}
    $O_t \gets \mathtt{Vary}(P_{t-1},\, \mathcal{A}[a_t])$\; \label{alg:aos:vary}
    $P_t \gets \mathtt{Survive}(P_{t-1} \cup O_t,\, N)$\; \label{alg:aos:survive}
    $n_{\mathrm{surv}} \gets |O_t \cap P_t|$\; \label{alg:aos:nsurv}
    $n_{\mathrm{nd}} \gets |\{x \in O_t \cap P_t : x \text{ non-dom.\ in } P_t\}|$\; \label{alg:aos:nnd}
    \tcc{Reward computation}
    $r_{\mathrm{surv}} \gets n_{\mathrm{surv}} \,/\, |O_t|$\; \label{alg:aos:rsurv}
    $r_{\mathrm{nd}} \gets n_{\mathrm{nd}} \,/\, |O_t|$\;
    $\rho_t \gets (\mathtt{HV}(P_t) - \mathtt{HV}(P_{t-1})) \,/\, \max(|\mathtt{HV}(P_{t-1})|,\, \epsilon)$\; \label{alg:aos:rho}
    $r_{\mathrm{hv}} \gets 0.5 + 0.5\,\tanh(\rho_t)$\;
    $r_t \gets w_{\mathrm{surv}}\, r_{\mathrm{surv}} + w_{\mathrm{nd}}\, r_{\mathrm{nd}} + w_{\mathrm{hv}}\, r_{\mathrm{hv}}$\; \label{alg:aos:reward}
    \tcc{Policy update}
    $\pi.\mathtt{update}(a_t,\, r_t)$\; \label{alg:aos:update}
}
\Return{$P_{\lfloor B/N \rfloor}$}\; \label{alg:aos:return}
\caption{\sc NSGA-II with generation-level AOS}
