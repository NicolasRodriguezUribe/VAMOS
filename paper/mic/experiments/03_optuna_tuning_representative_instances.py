"""
MIC Experiment 03: Optuna tuning on representative instances.

Workflow:
1) Read representative instances selected in Experiment 01.
2) Tune NSGA-II with Optuna on that subset.
...
from vamos.engine.tuning.racing.param_space import Boolean, Categorical, Int, ParamSpace, Real
...
def _resolve_output_dir(base: Path, name: str) -> Path:
    ...
    suffix = name.strip() if name.strip() else f"optuna_rep_instances_{ts}"
...
def _build_instances(selected_instances: list[str]) -> list[Instance]:
...
def _evaluate_config_at_budget(
    *,
    config: dict[str, Any],
    eval_fn: Any,
    instances: list[Instance],
    ...
    if not scores:
        raise RuntimeError("No scores computed for Optuna trial.")
    return float(np.mean(scores))


def _optuna_worker_process(cfg: _OptunaWorkerConfig) -> None:
    import optuna
    ...
    eval_fn = make_evaluator(engine=str(cfg.engine), failure_score=float(cfg.failure_score))
    instances = _build_instances(list(cfg.selected_instances))
    seeds = [int(s) for s in cfg.seeds]

    sampler = optuna.samplers.TPESampler(
        seed=int(cfg.seed) + int(cfg.worker_id) * 1009,
        multivariate=True,
        group=True,
    )
    pruner = optuna.pruners.MedianPruner(
        n_startup_trials=max(5, min(40, int(cfg.max_trials // 4) if cfg.max_trials > 0 else 5)),
        n_warmup_steps=max(1, len(cfg.budget_levels) - 1),
        interval_steps=1,
    )
    storage = optuna.storages.RDBStorage(
        url=str(cfg.storage_url),
    ...
    max_trials_cb = optuna.study.MaxTrialsCallback(
        int(cfg.max_trials),
        states=(TrialState.COMPLETE, TrialState.PRUNED, TrialState.FAIL),
    )

    def objective(trial: Any) -> float:
        config = _sample_from_optuna_trial(trial, param_space)
        param_space.validate(config)
        trial.set_user_attr("config", dict(config))
        final_score = float(cfg.failure_score)
        for step_idx, budget in enumerate(cfg.budget_levels):
            score = _evaluate_config_at_budget(
                config=config,
                eval_fn=eval_fn,
                instances=instances,
                seeds=seeds,
            ...
            trial.report(float(score), step=int(step_idx))
    ...
    study.optimize(
        objective,
        n_trials=None,
        timeout=float(cfg.timeout_seconds) if cfg.timeout_seconds is not None else None,
        n_jobs=1,
...
def _run_optuna_distributed(
    *,
    ...
    engine: str,
    failure_score: float,
) -> tuple[dict[str, Any], list[TrialResult], dict[str, Any]]:
    import optuna

    db_path = out_dir / "optuna_study.sqlite3"
    storage_url = f"sqlite:///{db_path.resolve().as_posix()}"
    study_name = f"{task.name}_{int(seed)}_{int(time.time())}"
    storage = optuna.storages.RDBStorage(
        url=storage_url,
        engine_kwargs={"connect_args": {"timeout": 120}},
    )

    base_sampler = optuna.samplers.TPESampler(seed=int(seed), multivariate=True, group=True)
    base_pruner = optuna.pruners.MedianPruner(
        n_startup_trials=max(5, min(40, int(max_trials // 4) if max_trials > 0 else 5)),
        n_warmup_steps=max(1, len(budget_levels) - 1),
        interval_steps=1,
    )
    _ = optuna.create_study(
        direction="maximize" if task.maximize else "minimize",
    ...
    cfgs = [
        _OptunaWorkerConfig(
            ...
            seed=int(seed),
            budget_levels=tuple(int(b) for b in budget_levels),
            selected_instances=tuple(str(x) for x in selected_instances),
            seeds=tuple(int(s) for s in seeds),
            engine=str(engine),
            failure_score=float(failure_score),
            show_progress_bar=bool(show_progress_bar and worker_id == 0),
        )
        for worker_id in range(workers)
    ]

    if workers == 1:
        _optuna_worker_process(cfgs[0])
    else:
        procs: list[mp.Process] = []
        for cfg in cfgs:
            proc = mp.Process(target=_optuna_worker_process, args=(cfg,))
            proc.start()
            procs.append(proc)
        for proc in procs:
            proc.join()
        ...
        if failed:
            details = ", ".join(f"pid={int(p.pid or -1)} exit={int(p.exitcode or -1)}" for p in failed)
            raise RuntimeError(f"One or more Optuna workers failed: {details}")
    ...
    if not history:
        raise RuntimeError("Optuna finished without a valid configuration.")

    best = max(history, key=lambda h: h.score) if task.maximize else min(history, key=lambda h: h.score)
    metadata = {
        "execution_mode": "optuna_distributed_processes" if workers > 1 else "optuna_single_process",
        "workers": int(workers),
        "study_name": study_name,
        "storage_url": storage_url,
        "storage_path": db_path.name,
    }
    return dict(best.config), history, metadata


def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="MIC Experiment 03: Optuna tuning on representative instances.")
    ap.add_argument(
        "--selection-file",
        type=str,
        default="",
    ...
    ap.add_argument("--max-trials", type=int, default=120, help="Total Optuna trials.")
    ...
    ap.add_argument("--output-dir", type=Path, default=Path("experiments") / "mic" / "optuna_tuning")
...
def main() -> None:
    ...
    optuna_available = bool(available.get("optuna", False))
    _logger().info("Optuna backend available: %s", optuna_available)
    ...
    if not optuna_available:
        raise RuntimeError(
            "Backend 'optuna' is unavailable. Install optional tuning dependencies first."
        )

    param_space = build_nsgaii_config_space().to_param_space()
    task = TuningTask(
        name="mic_optuna_nsgaii_representative_instances",
    ...
    _logger().info("Optuna execution mode: distributed process workers")
    _logger().info("Optuna workers: %s", n_jobs)

    t0 = time.perf_counter()
    best_config, history, optuna_meta = _run_optuna_distributed(
        task=task,
        out_dir=out_dir,
    ...
    summary = {
        "schema_version": "mic_optuna_tuning_v1",
        ...
        "execution_mode": str(optuna_meta.get("execution_mode", "")),
        "optuna_workers": int(optuna_meta.get("workers", n_jobs)),
        "optuna_study_name": str(optuna_meta.get("study_name", "")),
        ...
        "artifacts": {
            ...
            "optuna_study_db": str(optuna_meta.get("storage_path", "optuna_study.sqlite3")),
        },
    }
