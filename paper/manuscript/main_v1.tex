...
\toprule
\textbf{Feature} & \textbf{VAMOS} & \textbf{pymoo} & \textbf{jMetalPy} & \textbf{DEAP} & \textbf{Platypus} & \textbf{pygmo} \\
\midrule
Dominance-based algorithms          & \cmark & \cmark & \cmark & \pmark & \cmark & \pmark \\
Decomposition-based algorithms      & \cmark & \cmark & \pmark & \xmark & \cmark & \pmark \\
Indicator-based algorithms          & \cmark & \pmark & \pmark & \xmark & \xmark & \xmark \\
Pluggable computation backends      & \cmark & \xmark & \xmark & \xmark & \xmark & \xmark \\
Dense array population model        & \cmark & \pmark & \xmark & \xmark & \xmark & \xmark \\
Interchangeable variation operators & \cmark & \pmark & \pmark & \cmark & \pmark & \xmark \\
Configurable steady-state mode      & \cmark & \xmark & \pmark & \pmark & \xmark & \xmark \\
External archive support            & \cmark & \xmark & \xmark & \pmark & \pmark & \xmark \\
...
\begin{itemize}
  \item \textbf{NumPy}: baseline backend using vectorized array operations~\cite{HMV+20}.
  \item \textbf{Numba}: just-in-time (JIT) compilation of hot operators and utilities, reducing Python overhead~\cite{LPS15}.
  \item \textbf{MooCore}: optional C-backed kernels for Pareto ranking and hypervolume-based utilities~\cite{moocore24}.
\end{itemize}
...
\begin{table*}[htbp]
\centering
\caption{\textcolor{red}{Overview of the nine MOEAs implemented in \VAMOS{} and their configurable parameter spaces across all supported encodings. Encodings: R\,=\,real, P\,=\,permutation, B\,=\,binary, I\,=\,integer, M\,=\,mixed. The ``\#Cx'' and ``\#Mx'' columns report totals across supported encodings for each algorithm. The ``\#Params'' column reports the total number of configurable parameters (base + conditional) in the tuning configuration space. Acronyms: SBX (Simulated Binary Crossover), PM (Polynomial Mutation), DE (Differential Evolution), PBI (Penalty-based Boundary Intersection).}}
\label{tab:algo_overview}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llccclc}
\toprule
\textbf{Algorithm} & \textbf{Paradigm} & \textbf{Encodings} & \textbf{\#Cx} & \textbf{\#Mx} & \textbf{Specific Parameters} & \textbf{\#Params} \\
\midrule
...
Table~\ref{tab:parameter_space} summarizes the full parameter space considered for NSGA-II with real-valued encoding in \VAMOS{}, illustrating the depth of component-level configurability described in Section~\ref{sec:design_goals}. Of the 37 total parameters, 13 are always active (e.g., population size, crossover type, mutation probability factor) and 24 are conditional---activated only when their parent operator is selected (e.g., the distribution index $\eta_c$ applies only when SBX crossover is chosen). In any single configuration, between 15 and 20 parameters are simultaneously active, depending on the selected operators. The default configuration used throughout the experimental evaluation (Section~\ref{sec:experiments}) corresponds to the values shown in bold.
...
cfg = (NSGAIIConfig.builder()
       ...
       .build())
result = optimize("zdt1", algorithm="nsgaii", algorithm_config=cfg, max_evaluations=50000, seed=42)
\end{lstlisting}
...
problem = make_problem(my_objectives, n_var=2, n_obj=2, bounds=[(0, 2), (0, 2)])
result = optimize(problem, algorithm="nsgaii", max_evaluations=50000, seed=42)
\end{lstlisting}
