...
\begin{enumerate}
  ...
  \item \textbf{Rich component-level configurability}: nine MOEAs implemented on top of shared components, with interchangeable variation operators, configurable offspring batch size (enabling steady-state variants), optional bounded or unbounded external archives, multiple constraint-handling modes, and adaptive operator selection.
...
Table~\ref{tab:frameworks} summarizes high-level differences relevant to this work. The \emph{Config.}\ column reflects the depth of component-level configurability: \emph{Rich} denotes support for interchangeable variation operators, configurable offspring batch size (enabling steady-state variants), optional external archives, multiple constraint-handling modes, and adaptive operator selection; \emph{Medium} denotes partial support for some of these features; \emph{Low} denotes fixed or minimally configurable algorithmic pipelines. The \emph{LOC} column reports the number of lines of Python code required for a minimal NSGA-II run on a built-in benchmark problem: \VAMOS{} requires only two lines (Listing~\ref{lst:quickstart}), while other frameworks require importing and assembling multiple components. The main additional distinction is the internal representation: \VAMOS{} consistently uses dense arrays for populations and objectives, while most alternatives operate on per-solution objects. This difference enables kernel-based acceleration and reduces Python overhead in the inner loop.
...
\toprule
\textbf{Framework} & \textbf{MOEAs} & \textbf{Array} & \textbf{Accel.} & \textbf{Config.} & \textbf{LOC\textsuperscript{$\dagger$}} & \textbf{UI} \\
\midrule
\VAMOS{} & 9 & Yes & Numba/MooCore & Rich & 2 & Yes \\
pymoo~\cite{blank2020pymoo} & 8+ & Partial & No & Medium & $\sim$10 & No \\
jMetalPy~\cite{benitez2019jmetalpy} & 11+ & No & No & Rich & $\sim$15 & No \\
DEAP~\cite{fortin2012deap} & Custom & No & No & Low & $\sim$20 & No \\
Platypus~\cite{hadka2015platypus} & 8+ & No & No & Low & $\sim$8 & No \\
\bottomrule
...
The variation pipeline also supports adaptive operator selection (AOS), an optional layer that dynamically selects among a pool of crossover and mutation operators during the search based on reward signals derived from offspring quality. This mechanism enables the algorithm to adapt its operator configuration to the problem landscape without manual tuning.
