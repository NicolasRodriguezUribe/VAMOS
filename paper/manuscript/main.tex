...
\midrule
Numba & \textbf{7.09} & \textbf{6.58} & \textbf{7.14} & \textbf{6.94} \\
MooCore & 7.26 & 6.70 & 7.30 & 7.09 \\
NumPy & 13.68 & 12.58 & 12.63 & 12.96 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
VAMOS & \textbf{7.09} & \textbf{6.58} & \textbf{7.14} & \textbf{6.94} \\
pymoo & 9.10 & 8.19 & 8.29 & 8.53 \\
DEAP & 41.25 & 44.48 & 78.69 & 54.81 \\
jMetalPy & 33.96 & 29.87 & 78.64 & 47.49 \\
Platypus & 54.90 & 56.69 & 91.31 & 67.63 \\
\bottomrule
\end{tabular}
\end{table}
...
To better capture online adaptation effects under large evaluation budgets, we additionally report anytime performance by recording normalized hypervolume at fixed evaluation checkpoints (5k, 10k, 20k, 50k evaluations). To make early-budget differences explicit, we report median normalized hypervolume at 10k and 20k evaluations (Tables~\ref{tab:anytime_hv_10000} and \ref{tab:anytime_hv_20000}). From the same checkpoints we compute (i) AUC(HV vs.\ evals), and (ii) a time-to-target metric defined as the earliest checkpoint at which HV reaches $0.99\times$ the baseline-final median HV for that problem, along with the corresponding success rate by the full budget. In evaluation-dominated scenarios, lower evaluations-to-target translates into wall-clock savings. Tables~\ref{tab:anytime_auc}--\ref{tab:anytime_ttt_success} summarize these metrics aggregated by benchmark family.

We additionally log per-generation AOS arm selections and summarize stage-wise operator usage for representative problems to illustrate operator switching during the search (Figure~\ref{fig:aos_switching}).
...
\caption{Stage-wise AOS operator usage (median selection fraction across seeds) for representative problems (generated by \texttt{paper/10\_update\_anytime\_tables\_from\_csv.py} from \texttt{experiments/ablation\_aos\_trace.csv}).}
...
\midrule
Baseline & 0.88 & \textbf{0.92} & \textbf{0.98} & \textbf{0.93} \\
Baseline + AOS & \textbf{0.88} & 1.72 & 1.00 & 1.20 \\
Racing-tuned & 11.01 & 10.93 & 5.66 & 9.20 \\
Racing-tuned + AOS & 11.16 & 15.94 & 5.73 & 10.94 \\
\bottomrule
\end{tabular}
\end{table}
...
Baseline + AOS & 0.989 & 0.816 & 0.931 & 0.912 \\
Racing-tuned & \textbf{0.993} & \textbf{0.917} & 0.947 & \textbf{0.952} \\
Racing-tuned + AOS & 0.992 & 0.913 & \textbf{0.947} & 0.951 \\
\bottomrule
\end{tabular}
\end{table}
...
Baseline + AOS & 0.817 & 0.757 & 0.885 & 0.820 \\
Racing-tuned & \textbf{0.985} & 0.804 & \textbf{0.895} & 0.895 \\
Racing-tuned + AOS & 0.984 & \textbf{0.819} & 0.894 & \textbf{0.899} \\
\bottomrule
\end{tabular}
\end{table}
...
Baseline + AOS & 0.976 & 0.797 & 0.913 & 0.896 \\
Racing-tuned & 0.989 & 0.832 & \textbf{0.926} & 0.916 \\
Racing-tuned + AOS & \textbf{0.989} & \textbf{0.843} & 0.924 & \textbf{0.919} \\
\bottomrule
\end{tabular}
\end{table}
...
Baseline + AOS & 0.817 & 0.743 & 0.863 & 0.808 \\
Racing-tuned & 0.913 & 0.778 & \textbf{0.874} & 0.855 \\
Racing-tuned + AOS & \textbf{0.916} & \textbf{0.785} & 0.870 & \textbf{0.857} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Anytime ablation: median evaluations to reach $0.99\times$ the baseline-final HV target (per problem), aggregated by family; generated by \texttt{paper/10\_update\_anytime\_tables\_from\_csv.py}.}
\label{tab:anytime_ttt_evals}
\begin{tabular}{l|rrr|r}
\toprule
\textbf{Variant} & \textbf{ZDT} & \textbf{DTLZ} & \textbf{WFG} & \textbf{Average} \\
\midrule
Baseline & 20000 & 50000 & 50000 & 40000 \\
Baseline + AOS & 20000 & 50000 & 50000 & 40000 \\
Racing-tuned & \textbf{10000} & \textbf{10000} & \textbf{20000} & \textbf{13333} \\
Racing-tuned + AOS & \textbf{10000} & \textbf{10000} & \textbf{20000} & \textbf{13333} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Anytime ablation: success rate (\%) reaching the $0.99\times$ baseline-final HV target by the full budget, aggregated by family; generated by \texttt{paper/10\_update\_anytime\_tables\_from\_csv.py}.}
\label{tab:anytime_ttt_success}
\begin{tabular}{l|rrr|r}
\toprule
\textbf{Variant} & \textbf{ZDT} & \textbf{DTLZ} & \textbf{WFG} & \textbf{Average} \\
\midrule
Baseline & \textbf{100.0} & 81.0 & 85.2 & 88.7 \\
Baseline + AOS & \textbf{100.0} & \textbf{92.4} & 84.8 & \textbf{92.4} \\
Racing-tuned & \textbf{100.0} & 86.2 & 89.6 & 91.9 \\
Racing-tuned + AOS & 99.3 & 84.3 & \textbf{92.2} & 91.9 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item Scaling study:\newline \texttt{python}\ \path{paper/03_run_scaling_experiment.py}
\end{itemize}
...
\midrule
dtlz1 & 7.40 & \textbf{7.24} & 14.04 \\
dtlz2 & \textbf{6.75} & 6.98 & 13.04 \\
dtlz3 & 6.05 & \textbf{5.94} & 11.62 \\
dtlz4 & \textbf{6.29} & 6.38 & 11.39 \\
dtlz5 & \textbf{6.02} & 7.03 & 11.35 \\
dtlz6 & \textbf{6.14} & 6.54 & 11.42 \\
dtlz7 & \textbf{6.61} & 6.93 & 12.92 \\
wfg1 & \textbf{6.35} & 6.49 & 12.44 \\
wfg2 & 6.61 & \textbf{6.60} & 13.64 \\
wfg3 & \textbf{7.20} & 7.23 & 12.10 \\
wfg4 & \textbf{6.21} & 6.40 & 11.47 \\
wfg5 & 6.75 & \textbf{6.46} & 11.70 \\
wfg6 & \textbf{7.36} & 7.75 & 12.86 \\
wfg7 & \textbf{6.33} & 6.38 & 11.15 \\
wfg8 & \textbf{7.05} & 7.23 & 12.57 \\
wfg9 & \textbf{9.02} & 9.29 & 14.37 \\
zdt1 & \textbf{6.63} & 6.76 & 12.13 \\
zdt2 & 6.58 & \textbf{6.46} & 11.97 \\
zdt3 & \textbf{7.09} & 7.36 & 13.40 \\
zdt4 & \textbf{7.10} & 7.38 & 14.26 \\
zdt6 & \textbf{7.72} & 7.87 & 15.14 \\
\midrule
\textbf{Average} & \textbf{6.82} & 6.99 & 12.62 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
dtlz1 & \textbf{7.40} & 8.41 & 46.99 & 30.91 & 53.90 \\
dtlz2 & \textbf{6.75} & 8.40 & 47.69 & 30.38 & 59.61 \\
dtlz3 & \textbf{6.05} & 7.11 & 42.14 & 33.87 & 48.50 \\
dtlz4 & \textbf{6.29} & 7.23 & 46.02 & 26.55 & 54.57 \\
dtlz5 & \textbf{6.02} & 7.24 & 39.78 & 27.92 & 65.19 \\
dtlz6 & \textbf{6.14} & 8.69 & 36.32 & 29.76 & 53.42 \\
dtlz7 & \textbf{6.61} & 7.83 & 41.02 & 30.23 & 58.68 \\
wfg1 & \textbf{6.35} & 7.92 & 49.57 & 47.81 & 63.58 \\
wfg2 & \textbf{6.61} & 7.54 & 79.63 & 81.15 & 91.84 \\
wfg3 & \textbf{7.20} & 8.38 & 77.25 & 78.64 & 92.21 \\
wfg4 & \textbf{6.21} & 7.51 & 47.06 & 49.50 & 61.41 \\
wfg5 & \textbf{6.75} & 7.53 & 47.32 & 45.05 & 78.77 \\
wfg6 & \textbf{7.36} & 8.49 & 204.99 & 199.94 & 245.75 \\
wfg7 & \textbf{6.33} & 7.09 & 77.91 & 62.51 & 75.33 \\
wfg8 & \textbf{7.05} & 8.04 & 130.44 & 140.45 & 159.21 \\
wfg9 & \textbf{9.02} & 10.35 & 363.28 & 383.58 & 381.14 \\
zdt1 & \textbf{6.63} & 7.79 & 41.06 & 32.33 & 60.09 \\
zdt2 & \textbf{6.58} & 7.94 & 38.21 & 35.32 & 53.61 \\
zdt3 & \textbf{7.09} & 9.13 & 44.47 & 35.05 & 62.99 \\
zdt4 & \textbf{7.10} & 9.34 & 46.36 & 34.36 & 52.15 \\
zdt6 & \textbf{7.72} & 13.92 & 36.49 & 33.45 & 56.82 \\
\midrule
\textbf{Average} & \textbf{6.82} & 8.37 & 75.43 & 69.94 & 91.85 \\
\bottomrule
\end{tabular}
\end{table*}
