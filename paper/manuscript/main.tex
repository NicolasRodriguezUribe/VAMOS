...
% --------------------------------------------------------------------------
%  Author block.
%  If submitting to a double-anonymous venue, comment this block out.
% --------------------------------------------------------------------------
...
% --------------------------------------------------------------------------
\begin{abstract}
% Python has become the standard platform for empirical studies in
% multi-objective optimization, yet most framework libraries remain
% dominated by object-oriented designs that incur substantial interpreter
% overhead in the inner loop.
% We introduce a research-oriented framework that represents populations
% as contiguous numerical arrays and separates algorithmic logic from
% numerical kernels.
% The framework provides interchangeable compute backends, a unified
% configuration interface, and nine multi-objective evolutionary
% algorithms covering dominance-based, decomposition-based,
% indicator-based, and reference-vector paradigms.
% We also release a reproducible cross-framework benchmarking pipeline
% that aligns problem definitions and operator semantics across common
% Python frameworks, and reports wall-clock runtime together with
% normalized hypervolume computed against fixed reference Pareto fronts.
% Across widely used two- and three-objective benchmark suites, the
% proposed framework achieves up to an order-of-magnitude reduction in
% runtime relative to object-centric libraries, with more moderate but
% consistent gains over the closest competitor, while maintaining
% statistically equivalent solution quality as confirmed by paired
% non-parametric hypothesis tests with familywise error correction.
%
...
pure-Python implemented metaheuristics inherently slower than those of frameworks
...
pluggable backends (NumPy, Numba, C, JAX), keeping algorithmic logic
...
variation operators, population initialization strategies, and
optional bounded or unbounded external archives, exposed through a
minimal-boilerplate API accessible to domain experts with limited
Python experience.
The conducted experimentation across different scenarios, including runtime and solution quality analyses, highlights the performance of VAMOS and its different backends.
VAMOS is an open-source project available at \url{https://github.com/vamos-framework/vamos}.
%On the ZDT, DTLZ, and WFG benchmark suites with two and three objectives, VAMOS with its Numba backend reduces wall-clock runtime by $4$--$12\times$ relative to object-centric libraries and by $1.1$--$1.2\times$ relative to the closest array-aware competitor, with the advantage widening as population size grows. Paired Wilcoxon signed-rank tests with Holm correction confirm statistically equivalent solution quality on 20 of 21 problems
\end{abstract}

% Elsevier-only highlights (kept for reference; not used in IEEE submissions):
% \begin{highlights}
...
Multiobjective optimization, Vectorization
\end{IEEEkeywords}
...
Given the popularity of Python as a research platform, the use of Python MOEA frameworks can be difficult for users that are expert in the application domains but not in the multi-objective metaheuristics nor the optimization frameworks. A desirable property of these tools is to be easy to use for domain experts but also for users with a strong background in multi-objective optimization. 

We introduce \VAMOS{} (Vectorized Architecture for Multiobjective Optimization Studies), a Python-based package designed to address these limitations with a single guiding principle: \emph{keep algorithm state in dense arrays and push hot loops into vectorized kernels}. \VAMOS{} provides a highly efficient implementation of MOEA kernels with pluggable compute backends (NumPy, Numba, C, JAX) and includes a rich set of algorithmic features enabling configuration of solution generation, selection, variation, and the attachment of bounded or unbounded external archives. Additionally, \VAMOS{} exposes a minimal-boilerplate API for domain experts with limited Python experience and a graphical user interface to facilitate adoption.
...
\begin{enumerate}
  ...
  \item \textbf{Modular algorithm suite with rich configurability}: nine MOEAs implemented on top of shared components (termination, archives, evaluation backends, and metrics), with interchangeable variation operators, population initialization strategies, and optional bounded or unbounded external archives, all exposed through a minimal-boilerplate API accessible to domain experts with limited Python experience.
  \item \textbf{Graphical tool}: a Web-based interactive dashboard (\emph{VAMOS Studio}) for visual problem definition, live Pareto-front preview, and post-run analysis.
\end{enumerate}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Framework} & \textbf{MOEAs} & \textbf{Array} & \textbf{Accel.} & \textbf{UI} \\
\midrule
\VAMOS{} & 9 & Yes & Numba/C/JAX & Yes \\
pymoo~\cite{blank2020pymoo} & 8+ & Partial & No & No \\
jMetalPy~\cite{benitez2019jmetalpy} & 11+ & No & No & No \\
DEAP~\cite{fortin2012deap} & Custom & No & No & No \\
Platypus~\cite{hadka2015platypus} & 8+ & No & No & No \\
\bottomrule
\end{tabular}
\end{table}
...
Most frameworks require importing separate modules for the algorithm, problem, operators, and termination, then wiring them together (typically 10--15 lines for a standard Nondominated Sorting Genetic Algorithm~II (NSGA-II) run on a benchmark problem). \VAMOS{} exposes a single entry point (\texttt{optimize}) that handles defaults, operator configuration, and termination in a single call (Listing~\ref{lst:quickstart} in Appendix~\ref{app:code}).
For comparison, the equivalent run in pymoo requires separate imports for the algorithm class, problem factory, crossover, and mutation operators, explicit operator construction, and a \texttt{minimize} wrapper ($\sim$10 lines); jMetalPy additionally requires a termination-criterion object and explicit variable/objective counts ($\sim$15 lines). When finer control is needed, \VAMOS{} provides a builder API for algorithm configuration (Listing~\ref{lst:builder}).

\paragraph{Custom problem definition.}
Beyond running built-in benchmarks, users frequently need to optimize their own objective functions. Most frameworks require subclassing a problem base class and overriding an evaluation method, which adds boilerplate and framework coupling. \VAMOS{} provides a \texttt{make\_problem} factory that turns any Python callable into a problem object in four lines (Listing~\ref{lst:make_problem}), accepting plain functions, optional constraint functions, and handling adaptation automatically. An interactive command-line interface (CLI) wizard (\texttt{vamos create-problem}) can scaffold a ready-to-run script from a guided questionnaire.
...
\paragraph{Integrated tuning and ablation workflows.}
Algorithm performance is sensitive to hyperparameters, and fair comparisons often require systematic configuration rather than ad hoc defaults. \VAMOS{} therefore includes a tuning module (\texttt{vamos.engine.tuning}) exposed both programmatically and via the CLI (\texttt{vamos tune}). It supports random search and an irace-inspired racing procedure~\cite{lopez2016irace} with multi-fidelity evaluation (successive halving), warm-starting, and parallel execution; when available, the same interface can delegate to optional model-based backends such as Optuna~\cite{akiba2019optuna}, SMAC3~\cite{lindauer2022smac3}, and BOHB~\cite{falkner2018bohb}. The module also provides ablation-plan helpers for running controlled variants and summarizing results.
...
\begin{itemize}
  ...
  \item \textbf{JAX}: optional backend for parts of the pipeline and automatic-differentiation-based constraints, enabling GPU and tensor processing unit (TPU) acceleration when available~\cite{jax2018github}.
\end{itemize}
...
\begin{figure*}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/scalation.png}
...
\begin{figure*}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/time_comparisson.png}
...
\begin{itemize}
  ...
  \item \textbf{Distributed GPU execution}: expand JAX-based kernels and explore integration with EvoX~\cite{huang2025evox} for multi-node GPU acceleration.
...
% --------------------------------------------------------------------------
% DATA AVAILABILITY — commented out for double-anonymous review.
