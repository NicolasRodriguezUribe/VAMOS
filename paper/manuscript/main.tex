...
\midrule
NSGA-II & \cite{deb2002fast} & Dominance & R, P, B, I, M & 25 & 24 & Offspring ratio, repair, external archive (bounded/unbounded, pruning) & 35 \\
...
AGE-MOEA & \cite{panichella2019adaptive} & Dominance & R, P, B, I, M & 21 & 22 & External archive (bounded/unbounded, pruning) & 26 \\
...
%==============================================================================
In this section, we analyze the configurability of \VAMOS{} by examining the parameter space of NSGA-II with real-valued encoding. Table~\ref{tab:parameter_space} lists the 35 configurable parameters organized by component (general settings, initialization, variation, crossover, and mutation). Of these, 11 are always active (e.g., population size, boundary-repair strategy, crossover type and probability, mutation type and probability factor) and 24 are conditional, i.e., they are activated only when their parent operator is selected (e.g., the distribution index $\eta_c$ applies only when SBX crossover is chosen). In any single configuration, between 11 and 19 parameters are simultaneously active, depending on the selected operators. Tables~\ref{tab:parameter_space_pymoo} and~\ref{tab:parameter_space_jmetalpy} summarize the analogous constructor-level NSGA-II configuration surfaces of pymoo and jMetalPy, focusing on algorithmic knobs rather than auxiliary objects such as termination criteria, evaluators, or display hooks.
...
\multirow{7}{*}{General} & Pop.\ Size $\in \Intdomain{20, 200}$ (log-scale); default $\mathbf{100}$ & -- \\
 ...
 & Selection $\in$ \{\textbf{Tournament}, Random, Boltzmann, Ranking, SUS\} & -- \\
 & \textit{Tournament Size} $\in \Intdomain{\mathbf{2}, 10}$ & \textit{Selection} = Tournament \\
...
\multirow{1}{*}{Variation} & Boundary Repair $\in \{\textbf{clip}, reflect, random, round, wrap, midpoint\}$ & Applied after crossover and mutation \\ \midrule
...
\begin{table*}[htbp]
\centering
\caption{Built-in NSGA-II configuration surface in pymoo for real-valued problems. Unlike \VAMOS{}, pymoo configures NSGA-II by passing operator objects, so the table summarizes the built-in operator families shipped with the repository rather than a closed declarative tuning space. Defaults correspond to the standard NSGA-II constructor.}
\label{tab:parameter_space_pymoo}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lll}
\toprule
\textbf{Component} & \textbf{Value / Strategy} & \textbf{Conditional Parameters} \\ \midrule
\multirow{6}{*}{General} & Pop.\ Size $\in \mathbb{N}_{>0}$; default $\mathbf{100}$ & -- \\
 & Offspring Count $\in \mathbb{N}_{>0}$; default $\mathbf{pop\_size}$ & -- \\
 & Selection $\in$ \{\textbf{Tournament}, Random\} & -- \\
 & \textit{Tournament Pressure} $\in \mathbb{N}_{\ge 2}$; default $\mathbf{2}$ & \textit{Selection} = Tournament \\
 & Eliminate Duplicates $\in \{\textbf{True}, False\}$ & -- \\
 & External Archive & Not available in the standard NSGA-II API \\ \midrule
\multirow{4}{*}{Initialization} & \{\textbf{Random}, LHS\} & -- \\
 & \textit{LHS Smooth} $\in \{\textbf{True}, False\}$ & \textit{Sampling} = LHS \\
 & \textit{LHS Iterations} $\in \mathbb{N}_{>0}$; default $\mathbf{20}$ & \textit{Sampling} = LHS \\
 & \textit{LHS Criterion} $\in \{\textbf{maxmin}, corr, none\}$ & \textit{Sampling} = LHS \\ \midrule
\multirow{1}{*}{Variation} & Repair $\in \{\textbf{none}, to\_bound, bounce\_back, inverse\_penalty\}$ & Optional global repair; operator-local bound handling may still apply \\ \midrule
\multirow{4}{*}{Crossover} & \multicolumn{2}{l}{\textit{Global:} Probability $\in [0, 1]$} \\
 & \textbf{SBX} & $\eta$, $p_{\mathrm{var}}$, $p_{\mathrm{exch}}$, $p_{\mathrm{bin}}$, \textit{offsprings} $\in \{1, 2\}$ \\
 & PCX & $\eta$, $\zeta$ \\
 & DEX & $F$, $CR$, \textit{variant} $\in \{\text{bin}, \text{exp}\}$, \textit{n\_diffs}, \textit{dither}, \textit{jitter}, \textit{n\_iter} \\ \midrule
\multirow{3}{*}{Mutation} & \multicolumn{2}{l}{\textit{Global:} Probability $\in [0, 1]$, Prob.\ Var.\ $\in [0, 1]$ (default $\min(0.5, 1/n)$)} \\
 & \textbf{Polynomial} & $\eta$, \textit{at\_least\_once} $\in \{\textbf{False}, True\}$ \\
 & Gaussian & $\sigma$ \\ \bottomrule
\end{tabular}%
}
\end{table*}
...
\begin{table*}[htbp]
\centering
\caption{Built-in NSGA-II configuration surface in jMetalPy for real-valued problems. As in pymoo, NSGA-II is configured by supplying operator objects; the table summarizes the real-valued operators shipped with the repository rather than a closed tuning space. Defaults correspond to the standard NSGA-II constructor.}
\label{tab:parameter_space_jmetalpy}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lll}
\toprule
\textbf{Component} & \textbf{Value / Strategy} & \textbf{Conditional Parameters} \\ \midrule
\multirow{5}{*}{General} & Pop.\ Size $\in \mathbb{N}_{>0}$ & -- \\
 & Offspring Pop.\ Size $\in \mathbb{N}_{>0}$; steady-state with 1 & -- \\
 & Selection $\in$ \{\textbf{Binary Tournament}, Tournament, Random, Roulette Wheel, Best Solution, Binary Tournament 2\} & -- \\
 & \textit{Tournament Size} $\in \mathbb{N}_{\ge 2}$; default $\mathbf{2}$ & \textit{Selection} = Tournament \\
 & External Archive & Not available in the standard NSGA-II API \\ \midrule
\multirow{1}{*}{Initialization} & Population Generator = custom generator; default RandomGenerator & -- \\ \midrule
\multirow{1}{*}{Variation} & Float Repair $\in \{\textbf{clamp}, random\_uniform, reflective, bound\_swap\}$ & Passed inside float crossover/mutation operators; no separate global repair stage \\ \midrule
\multirow{6}{*}{Crossover} & \textbf{SBX} & Probability, Dist.\ Index, optional repair \\
 & BLX-$\alpha$ & Probability, $\alpha$, optional repair \\
 & BLX-$\alpha\beta$ & Probability, $\alpha$, $\beta$, optional repair \\
 & Arithmetic & Probability, optional repair \\
 & UNDX & Probability, $\zeta$, $\eta$, optional repair \\
 & Differential Evolution & $CR$, $F$, $K$ \\ \midrule
\multirow{6}{*}{Mutation} & \textbf{Polynomial} & Probability, Dist.\ Index, optional repair \\
 & Simple Random & Probability \\
 & Uniform & Probability, Perturbation, optional repair \\
 & Non-Uniform & Probability, Perturbation, Max Iterations, optional repair \\
 & L\'evy Flight & Probability, $\beta$, Step Size, optional repair \\
 & Power Law & Probability, $\delta$, optional repair \\ \bottomrule
\end{tabular}%
}
\end{table*}
