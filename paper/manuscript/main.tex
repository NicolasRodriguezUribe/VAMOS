...
%==============================================================================
\section{NSGA-II Parameter Space}
\label{app:param_space}
%==============================================================================

\textcolor{red}{Table~\ref{tab:parameter_space} summarizes the full parameter space considered for NSGA-II in \VAMOS{}, illustrating the depth of component-level configurability described in Section~\ref{sec:design_goals}. The default configuration used throughout the experimental evaluation (Section~\ref{sec:experiments}) corresponds to the values shown in bold.}

\begin{table}[htbp]
\centering
\caption{Summary of the considered parameter space for NSGA-II. Parameters in italics are conditional, active only when their corresponding operator is selected. Values in \textbf{bold} indicate the default NSGA-II configuration. The total number of configurable parameters is 32. Acronyms: LHS (Latin Hypercube Sampling), SUS (Stochastic Universal Sampling), SBX (Simulated Binary Crossover), BLX (Blend Crossover), PCX (Parent Centric Crossover), UNDC (Unimodal Normal Distribution Crossover).}
\label{tab:parameter_space}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lll}
\toprule
\textbf{Component} & \textbf{Value / Strategy} & \textbf{Conditional Parameters} \\ \midrule
\multirow{4}{*}{General} & Offspring Pop.\ Size $\in \{1, 5, 10, 20, 50, \mathbf{100}, 200, 400\}$ & -- \\
 & Algorithm Result $\in$ \{\textbf{Population}, ExternalArchive\} & -- \\
 & \textit{Archive Type} $\in$ \{CrowdingDistance, Unbounded\} & \textit{Result} = ExternalArchive \\
 & \textit{Pop.\ Size (Archive)} $\in \Intdomain{10, 200}$ & \textit{Result} = ExternalArchive \\ \midrule
Initialization & \{\textbf{Default}, LHS, Scatter, Chaos, Grid, Opposition\} & -- \\ \midrule
\multirow{3}{*}{Selection} & \textbf{Tournament} & \textit{Size} $\in \Intdomain{\mathbf{2}, 10}$ \\
 & Boltzmann & \textit{Temperature} $\in \Realdomain{0.1, 100.0}$ \\
 & Random, Ranking, SUS & -- \\ \midrule
\multirow{11}{*}{Crossover} & \multicolumn{2}{l}{\textit{Global:} Probability $\in \Realdomain{0.0, 1.0}$, Repair $\in$ \{random, round, \textbf{bounds}\}} \\
 & \textbf{SBX} & \textit{Dist.\ Index} $\in \Realdomain{5.0, 400.0}$ \\
 & BLX-$\alpha$ & $\alpha \in \Realdomain{0.0, 1.0}$ \\
 & BLX-$\alpha\beta$ & $\alpha, \beta \in \Realdomain{0.0, 1.0}$ \\
 & Laplace & \textit{Scale} $\in \Realdomain{0.1, 0.5}$ \\
 & Fuzzy & $\alpha \in \Realdomain{0.0, 1.0}$ \\
 & PCX & $\zeta, \eta \in \Realdomain{0.0, 1.0}$ \\
 & UNDC & $\zeta \in \Realdomain{0.1, 1.0}, \eta \in \Realdomain{0.1, 0.5}$ \\
 & WholeArithmetic, Arithmetic & -- \\ \midrule
\multirow{7}{*}{Mutation} & \multicolumn{2}{l}{\textit{Global:} Prob.\ Factor $\in \Realdomain{0.0, 2.0}$, Repair $\in$ \{random, round, \textbf{bounds}\}} \\
 & Uniform & \textit{Perturbation} $\in \Realdomain{0.0, 1.0}$ \\
 & NonUniform & \textit{Perturbation} $\in \Realdomain{0.0, 1.0}$ \\
 & \textbf{Polynomial} & \textit{Dist.\ Index} $\in \Realdomain{5.0, 400.0}$ \\
 & LinkedPolynomial & \textit{Dist.\ Index} $\in \Realdomain{5.0, 400.0}$ \\
 & LevyFlight & $\beta \in \Realdomain{1.0, 2.0}, \textit{Step} \in \Realdomain{0.01, 1.0}$ \\
 & PowerLaw & $\Delta \in \Realdomain{0.1, 10.0}$ \\ \bottomrule
\end{tabular}%
}
\end{table}
...
\textcolor{red}{Listing~\ref{lst:optuna_tune} shows how to autotune the full NSGA-II parameter space (Table~\ref{tab:parameter_space}) using the Optuna backend. The \texttt{build\_nsgaii\_config\_space} function returns the 32-parameter space; \texttt{ModelBasedTuner} drives the search with TPE sampling and optional multi-fidelity evaluation.}

\begin{lstlisting}[caption={\textcolor{red}{Autotuning NSGA-II with Optuna on ZDT1.}},label={lst:optuna_tune}]
import numpy as np
from vamos import optimize
...
# Build the 32-parameter NSGA-II config space
