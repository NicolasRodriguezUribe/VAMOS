...
To evaluate the practical impact of \VAMOS{}' optional adaptation modules beyond the cross-framework baseline, we run an internal ablation study on the same benchmark suite and evaluation budgets as the runtime experiments. We compare four \VAMOS{} variants of NSGA-II in a $2\times2$ factorial design (tuned vs.\ untuned) $\times$ (AOS disabled vs.\ enabled):
\begin{enumerate}
  ...
  \item \textbf{Baseline + AOS}: the same configuration augmented with adaptive operator selection (AOS), where an $\epsilon$-greedy bandit selects between a small portfolio of real-coded variation pipelines (SBX+PM vs.\ BLX-$\alpha$+PM). Rewards combine survival and non-dominated insertion rates with a lightweight per-generation hypervolume-change term (computed when inexpensive, up to three objectives) to reduce the risk of selecting operators that improve selection statistics while degrading Pareto-front quality.
  ...
  \item \textbf{Racing-tuned + AOS}: the racing-tuned configuration augmented with the same AOS portfolio and bandit policy, to quantify whether online operator adaptation provides additional benefit beyond offline tuning.
\end{enumerate}
The selected racing-tuned configuration is exported for reproducibility (\path{experiments/tuned_nsgaii_resolved.json}) and summarized in Table~\ref{tab:racing_tuned_config}. All variants are evaluated for 30 independent seeds and reported using the same normalized hypervolume protocol (Section~\ref{sec:hv}) and wall-clock runtime measurement setup. Tables~\ref{tab:ablation_runtime} and~\ref{tab:ablation_hypervolume} summarize the final-budget ablation outcomes aggregated by benchmark family.

To better capture online adaptation effects under large evaluation budgets, we additionally report anytime performance by recording normalized hypervolume at fixed evaluation checkpoints (5k, 10k, 20k, 50k, 100k evaluations). From these checkpoints we compute (i) AUC(HV vs.\ evals), and (ii) a time-to-target metric defined as the earliest checkpoint at which HV reaches $0.99\times$ the baseline-final median HV for that problem, along with the corresponding success rate by the full budget. Tables~\ref{tab:anytime_auc}--\ref{tab:anytime_ttt_success} summarize these metrics aggregated by benchmark family.
...
\midrule
\texttt{pop\_size} & 183 \\
\texttt{offspring\_size} & 183 \\
Crossover & \texttt{sbx(eta=29.02, prob=0.9859)} \\
Mutation & \texttt{non\_uniform(prob=0.1, prob\_factor=0.5029)} \\
Selection & \texttt{tournament(pressure=2)} \\
External archive & disabled \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Baseline & \textbf{1.75} & 1.90 & \textbf{1.95} & \textbf{1.87} \\
Baseline + AOS & 1.79 & \textbf{1.87} & 1.97 & 1.88 \\
Racing-tuned & 76.15 & 74.57 & 17.92 & 56.21 \\
Racing-tuned + AOS & 37.01 & 68.77 & 18.89 & 41.56 \\
\bottomrule
\end{tabular}
\end{table}
...
Baseline + AOS & 0.990 & 0.828 & 0.937 & 0.918 \\
Racing-tuned & \textbf{0.995} & \textbf{0.934} & 0.858 & 0.929 \\
Racing-tuned + AOS & 0.995 & 0.921 & 0.906 & \textbf{0.941} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Variant} & \textbf{ZDT} & \textbf{DTLZ} & \textbf{WFG} & \textbf{Average} \\
\midrule
Baseline & 0.909 & 0.763 & \textbf{0.900} & 0.857 \\
Baseline + AOS & 0.900 & 0.772 & 0.899 & 0.857 \\
Racing-tuned & \textbf{0.942} & \textbf{0.830} & 0.814 & \textbf{0.862} \\
Racing-tuned + AOS & 0.899 & 0.820 & 0.865 & 0.862 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Variant} & \textbf{ZDT} & \textbf{DTLZ} & \textbf{WFG} & \textbf{Average} \\
\midrule
Baseline & \textbf{20000} & 50000 & 50000 & 40000 \\
Baseline + AOS & \textbf{20000} & 50000 & 50000 & 40000 \\
Racing-tuned & \textbf{20000} & \textbf{10000} & 50000 & \textbf{26667} \\
Racing-tuned + AOS & 50000 & 20000 & \textbf{20000} & 30000 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Variant} & \textbf{ZDT} & \textbf{DTLZ} & \textbf{WFG} & \textbf{Average} \\
\midrule
Baseline & \textbf{100.0} & 88.6 & 86.7 & 91.7 \\
Baseline + AOS & \textbf{100.0} & 93.3 & 85.6 & 93.0 \\
Racing-tuned & \textbf{100.0} & 96.7 & 78.9 & 91.9 \\
Racing-tuned + AOS & \textbf{100.0} & \textbf{100.0} & \textbf{91.1} & \textbf{97.0} \\
\bottomrule
\end{tabular}
\end{table}
