...
\midrule
VAMOS & \textbf{13.73} & \textbf{21.80} & \textbf{32.38} \\
...
\midrule
NSGA-II (steady-state) & VAMOS & \textbf{163.99} & \textbf{252.57} & \textbf{675.29} \\
 & pymoo & 5747.47 & 7259.88 & 6382.73 \\
 & jMetalPy & 1930.43 & 2506.83 & 2756.07 \\
\midrule
NSGA-II (ext. archive) & VAMOS & \textbf{726.37} & \textbf{3744.19} & \textbf{543.79} \\
 & pymoo & 1422.71 & 7208.12 & 1515.89 \\
 & jMetalPy & 744.93 & 3993.47 & 1736.74 \\
\midrule
SMS-EMOA & VAMOS & \textbf{128.32} & \textbf{209.10} & \textbf{685.98} \\
 & pymoo & 1658.10 & 2439.60 & 3410.49 \\
 & jMetalPy & 377.01 & 529.49 & 1161.51 \\
\midrule
MOEA/D & VAMOS & 537.56 & 885.51 & \textbf{2858.06} \\
 & pymoo & 1325.29 & 1735.46 & 4143.63 \\
 & jMetalPy & \textbf{303.00} & \textbf{430.14} & 4267.52 \\
\bottomrule
\end{tabular}
\end{table}

\VAMOS{} is the fastest framework for the steady-state NSGA-II, external archive, and SMS-EMOA configurations across all problem families. The speedup is most pronounced for the steady-state variant, where \VAMOS{} is 9.5--35$\times$ faster than pymoo and 4--12$\times$ faster than jMetalPy. SMS-EMOA shows a similar pattern (5--13$\times$ over pymoo, 1.7--2.9$\times$ over jMetalPy). These large margins are expected: both algorithms execute $N$ iterations per generation, amplifying the per-iteration overhead of object-oriented frameworks. For the external archive variant, the advantage is more moderate (about 2$\times$ over pymoo), since the dominant cost shifts to archive maintenance rather than per-iteration dispatch. For MOEA/D, \VAMOS{} remains 1.4--2.5$\times$ faster than pymoo, but jMetalPy is 1.5--2$\times$ faster than \VAMOS{} on ZDT and DTLZ, indicating that jMetalPy's MOEA/D implementation has lower per-iteration overhead for these lower-dimensional families. On WFG, \VAMOS{} recovers the advantage.
...
\toprule
\textbf{Family} & \textbf{VAMOS} & \textbf{pymoo} & \textbf{jMetalPy} & \textbf{DEAP} & \textbf{Platypus} \\
\midrule
ZDT & \textbf{0.991 (0.013)} & \textbf{0.991 (0.010)} & \textbf{0.991 (0.014)} & \textbf{0.991 (0.013)} & \textbf{0.991 (0.010)} \\
DTLZ & 0.836 (0.121) & 0.829 (0.148) & 0.835 (0.110) & \textbf{0.839 (0.138)} & 0.824 (0.177) \\
WFG & \textbf{0.934 (0.138)} & 0.846 (0.220) & 0.845 (0.223) & 0.839 (0.224) & 0.919 (0.137) \\
\midrule
Overall & \textbf{0.934 (0.157)} & 0.921 (0.156) & 0.924 (0.159) & 0.924 (0.156) & 0.919 (0.160) \\
\bottomrule
\end{tabular}
\end{table}
...
Not all comparisons favor \VAMOS{}. For MOEA/D on ZDT and DTLZ (Table~\ref{tab:frameworks_perf_variants}), jMetalPy is 1.5--2$\times$ faster than \VAMOS{}. This is attributable to differences in the decomposition inner loop: jMetalPy's MOEA/D processes one subproblem at a time with lightweight scalar operations, a pattern where Python-level overhead is modest and \VAMOS{}'s array-batching strategy does not yield the same advantage as in generational algorithms with population-wide survival. On WFG, where higher dimensionality amplifies per-solution overhead, \VAMOS{} recovers the advantage.
