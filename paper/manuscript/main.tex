...
\title{VAMOS: A Vectorized Architecture for Multiobjective Optimization Studies\\A High-Performance Python Framework for Fair Cross-Framework Benchmarking}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Framework} & \textbf{MOEAs} & \textbf{Array core} & \textbf{Accel. kernels} & \textbf{Auto-conf.} \\
\midrule
...
\begin{figure}[htbp]
  ...
  \caption{Illustration of population representation differences: \VAMOS{} stores the full population in dense arrays and applies operators via array-oriented kernels (NumPy/Numba/C/JAX), while object-centric frameworks manage collections of per-solution records and perform selection/variation/survival through object/container-level workflows.}
...
We control the number of seeds, evaluation budget, and workers with \path{VAMOS_N_SEEDS}, \path{VAMOS_N_EVALS}, and \path{VAMOS_N_JOBS}; unless otherwise stated, we use \path{VAMOS_N_SEEDS=30} and \path{VAMOS_N_EVALS=50000}.

Paper tables are regenerated by:
\begin{itemize}
  \item Runtime tables:\newline \texttt{python}\ \path{paper/04_update_paper_tables_from_csv.py}
  \item Framework-variant runtime tables:\newline \texttt{python}\ \path{paper/14_update_frameworks_perf_variant_tables_from_csv.py}
\end{itemize}

Benchmark reports also emit jMetalPy-compatible laboratory outputs (e.g., \path{QualityIndicatorSummary.csv} and boxplots) under:\newline \path{summary/lab/}, following the laboratory summary format described by jMetalPy~\cite{benitez2019jmetalpy}.
...
Figure~\ref{fig:methodology_pro} summarizes the propose--apply--verify loop we used to keep changes auditable while building the benchmark runner and table-generation pipeline.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/methodology_pro.png}
  \caption{Agentic propose--apply--verify loop used during development: propose a targeted change, implement a patch, run fast-fail checks, execute the benchmark pipeline, and iterate until gates pass.}
  \label{fig:methodology_pro}
\end{figure}

As a development aid within this workflow, we used a large language model (LLM) to accelerate mechanical refactors and surface configuration mismatches across frameworks; proposed changes were applied as explicit diffs, reviewed by the authors, and accepted only if they passed automated checks. This does not affect the experimental results, which are produced solely by the released code and fixed reference data.
