...
Active maintenance                  & \cmark & \pmark & \xmark & \xmark & \cmark & \cmark \\
\bottomrule
\end{tabular}
\end{table*}
...
\paragraph{Scope of the extended comparison.}
DEAP~\cite{fortin2012deap} and Platypus are included in the generational NSGA-II runtime table above for completeness, but are deliberately excluded from the extended experimental analyses (solution-quality comparisons, IGD$^+$ metrics, and algorithm-variant benchmarks).
DEAP is a general-purpose evolutionary computation toolkit whose NSGA-II implementation prioritizes pedagogical clarity over computational efficiency, while Platypus has seen little active maintenance and has a substantially smaller adoption in the MOEA community.
Both frameworks ran more than one order of magnitude slower than pymoo on our benchmarks in preliminary tests, making them unsuitable as primary performance-comparison points; retaining them in all analyses would also have multiplied the total experiment time by a factor incompatible with the study's scale.
The extended comparisons are therefore restricted to pymoo~\cite{blank2020pymoo} and jMetalPy~\cite{benitez2019jmetalpy}, which represent the current state of the art in optimized Python MOEA implementations.
