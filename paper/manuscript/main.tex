...
\thanks{N. R. Uribe, A. Herr\'{a}n, and J. M. Colmenar are with the Dept. Computer Sciences, Universidad Rey Juan Carlos, M\'{o}stoles, 28933 Madrid, Spain (e-mail: nicolas.rodriguez@urjc.es; alberto.herran@urjc.es; josemanuel.colmenar@urjc.es).}%
\thanks{A. J. Nebro is with the Dept. de Lenguajes y Ciencias de la Computaci\'{o}n, ITIS Software, University of M\'{a}laga, 29071 M\'{a}laga, Spain (e-mail: ajnebro@uma.es).}%
\thanks{J. del Ser is with TECNALIA, Basque Research \& Technology Alliance (BRTA), Derio, 48160, Spain, and also with the Dept. of Mathematics, University of the Basque Country (UPV/EHU), Leioa, 48940, Spain (e-mail: javier.delser@tecnalia.com).}%
...
\begin{IEEEkeywords}
Multi-objective optimization, evolutionary algorithms, benchmarking, hypervolume, Python framework, vectorization
\end{IEEEkeywords}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Framework} & \textbf{MOEAs} & \textbf{Array core} & \textbf{Accel. kernels} & \textbf{Interactive UI} \\
\midrule
\VAMOS{} & 9 & Yes & Numba/C/JAX & Yes \\
pymoo~\cite{blank2020pymoo} (0.6) & 8+ & Partial & No & No \\
jMetalPy~\cite{benitez2019jmetalpy} (2019) & 11+ & No & No & No \\
DEAP~\cite{fortin2012deap} (1.4) & Custom & No & No & No \\
...
Beyond running built-in benchmarks, users frequently need to optimize their own objective functions. Most frameworks require subclassing a problem base class and overriding an evaluation method, which adds boilerplate and framework coupling. \VAMOS{} provides a convenience factory that turns any Python callable into a problem object:

\begin{lstlisting}[caption={Defining and optimizing a custom problem in four lines.},label={lst:make_problem}]
...
\noindent The \texttt{make\_problem} factory accepts plain functions, supports optional constraint functions, and handles the adaptation to the internal problem protocol automatically. An interactive CLI wizard (\texttt{vamos create-problem}) can scaffold a ready-to-run script from a guided questionnaire, further lowering the entry barrier for new users.
...
\caption{\VAMOS{} four-layer architecture. The Foundation layer provides problem definitions, compute kernels (NumPy, Numba, C, JAX), metrics, and archives. The Engine layer implements nine MOEAs using shared selection, variation, and survival components. The Experiment layer exposes CLI tools, benchmarking runners, table/plot generation, and results analysis. The UX layer provides VAMOS Studio, an interactive dashboard for visual problem building, onboarding, and results exploration.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/scalation.png}
