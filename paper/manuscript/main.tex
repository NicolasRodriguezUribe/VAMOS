...
browser-based graphical interface make the framework accessible to
domain experts with limited Python experience.
On the ZDT, DTLZ, and WFG benchmark suites with two and three objectives, VAMOS with its Numba backend reduces wall-clock runtime by $4$--$12\times$ relative to object-centric libraries and by $1.1$--$1.2\times$ relative to the closest array-aware competitor, with the advantage widening as population size grows. Paired Wilcoxon signed-rank tests with Holm correction confirm statistically equivalent or superior solution quality across frameworks.
...
\IEEEPARstart{M}{ulti-objective} optimization problems (MOPs) arise whenever trade-offs exist between conflicting objectives, requiring the computation of a diverse approximation of the Pareto-optimal set rather than a single optimum~\cite{Deb01}. Such problems are pervasive in engineering design, machine learning, scheduling, and resource allocation, among other domains. In this field, multi-objective evolutionary algorithms (MOEAs) remain a dominant approach due to their robustness, population-based anytime behavior, and ability to return a set of non-dominated solutions in a single run~\cite{coello2007evolutionary}.
...
As Python has become a widely adopted platform for MOEA research and experimentation, three practical limitations hinder its broader adoption. First, Python's interpreted nature makes pure-Python inner loops inherently slower than those of frameworks written in compiled languages such as C++ or Java. Second, most Python MOEA libraries further aggravate this overhead by managing populations as collections of per-solution objects, which prevents effective vectorization and keeps hot-path computations, that is selection, variation, and survival inside the interpreter~\cite{benitez2019jmetalpy,fortin2012deap,hadka2015platypus}. 

A related issue that affects most of MOEA frameworks, not only Python-based ones, is the limited configurability of the algorithms. If we focus on NSGA-II~\cite{deb2002fast}, for instance, most of the tools provide a small set of commonly used parameters, such as the population size, the number of generations, or the crossover and mutation probabilities and distribution indexes of the SBX and polynomial mutation operators. However, features such as setting the offspring population size (e.g., to configure a steady-state variant if that size is 1) or the use of external archives, that has been shown to be a useful mechanism to allow NSGA-II to handle problems with more than two objectives~\cite{IshibuchiPS20}, are not included in most of the tools with the exception of jMetal~\cite{durillo2011jmetal}. 

A third limitation, often overlooked, is the entry barrier for domain experts who are not specialists in multi-objective optimization. Practitioners in fields such as biology, physics, or engineering increasingly face multi-objective trade-offs in their work, yet existing frameworks typically require importing and wiring together multiple modules (algorithm, problem, operators, termination), understanding framework-specific abstractions, and writing 10--15 lines of boilerplate code for a standard run. This overhead discourages adoption by users with basic Python skills who would otherwise benefit from MOEA-based analysis.

We introduce \VAMOS{}, a Python-based package designed to address these three limitations---runtime overhead, limited configurability, and accessibility---with a single guiding principle: \emph{keep algorithm state in dense arrays and push hot loops into vectorized kernels}. \VAMOS{} provides a highly efficient implementation of MOEA kernels with pluggable compute backends (NumPy, Numba, MooCore/C) and includes a rich set of algorithmic features enabling configuration of solution generation, selection, variation, and the attachment of bounded or unbounded external archives. Equally important, \VAMOS{} is designed to be usable by non-specialists: a single two-line call (\texttt{optimize("zdt1", algorithm="nsgaii")}) runs a complete optimization; a \texttt{make\_problem} factory turns any Python function into a problem without subclassing; and a browser-based graphical interface (\emph{VAMOS Studio}) with domain-specific templates allows users to define and solve problems without writing any code. The contributions of this work are:
\begin{enumerate}
  ...
  \item \textbf{Accessible API and tooling for non-expert users}: a two-line entry point for complete optimization runs, a \texttt{make\_problem} factory that turns any Python callable into a problem without subclassing, automatic algorithm selection from problem traits, a CLI wizard for guided setup, and a Web-based interactive dashboard (\emph{VAMOS Studio}) with domain-specific templates for users with no programming experience.
\end{enumerate}
...
Several mature Python libraries exist for MOEAs. \textbf{pymoo}~\cite{blank2020pymoo} provides a rich collection of algorithms and operators with an emphasis on modularity; however, it does not expose fine-grained configuration of offspring batch size (precluding native steady-state variants), does not support attaching external archives to arbitrary algorithms, and lacks pluggable compute backends. \textbf{jMetalPy}~\cite{benitez2019jmetalpy} ports the jMetal architecture to Python and emphasizes end-to-end experimentation (observer-driven progress/visualization, a ``laboratory'' workflow, and built-in statistical post-processing with LaTeX export). It additionally offers modules for constrained, dynamic, and preference-based variants, as well as parallel evaluation via Spark/Dask. However, it relies on object-centric representations and does not address cross-toolkit semantic drift. We build upon this methodological focus on rigorous experimental studies and complement it with an array-based execution model and a fairness-oriented benchmarking protocol that aligns problem definitions and operator semantics across Frameworks. \textbf{DEAP}~\cite{fortin2012deap} is a general evolutionary computation toolkit; while flexible, users typically assemble MOEAs manually. \textbf{Platypus}~\cite{hadka2015platypus} provides a lightweight API and a set of classic MOEAs.

On the infrastructure side, EvoX~\cite{huang2025evox} targets automated, distributed, and heterogeneous execution of evolutionary computation, combining a functional programming model with hierarchical state management to enable graphics processing unit (GPU)-accelerated, multi-node runs. While EvoX primarily addresses system-level scalability for general evolutionary computation workloads, \VAMOS{} targets a complementary gap: a vectorized, backend-agnostic architecture optimized for multi-objective optimization studies, with emphasis on reproducible cross-framework benchmarking and accessible experimental workflows.

Beyond general-purpose MOEA libraries, recent research has introduced specialized frameworks and workflows: a coevolutionary approach for constrained multiobjective optimization using helper problems and information sharing~\cite{tian2021coevolutionary}; an evolutionary multitask framework for multimodal optimization with explicit/implicit bi-knowledge transfer between species~\cite{zhao2026multitask}; Light-EvoOPT, a four-stage pipeline for ultralarge-scale mixed-integer linear programs combining problem division, model-based initialization, reduction, and evolutionary improvement~\cite{ye2026lightevoopt}; and LLaMoCo, which instruction-tunes large language models to generate optimization code~\cite{ma2026llamoco}. These contributions focus on specific problem settings or automation, whereas \VAMOS{} targets a vectorized, backend-agnostic architecture for multi-objective optimization studies and reproducible cross-framework benchmarking.

Table~\ref{tab:frameworks} summarizes high-level differences relevant to this work. The \emph{Array} column indicates whether the framework stores populations and objectives in dense numerical arrays (\emph{Yes}), uses arrays partially while still relying on per-solution objects (\emph{Partial}), or operates entirely on individual solution objects (\emph{No}); this distinction is the primary determinant of inner-loop overhead, as array-native representations enable vectorized operations and compiled kernels. The \emph{Accel.}\ column lists available acceleration backends beyond pure Python: Numba  and MooCore in \VAMOS{}; MooCore only in pymoo; and none in the remaining frameworks. The \emph{Config.}\ column reflects the depth of component-level configurability: \emph{Rich} denotes support for interchangeable variation operators, configurable offspring batch size (enabling steady-state variants), optional external archives, multiple constraint-handling modes; \emph{Medium} denotes partial support for some of these features; \emph{Low} denotes fixed or minimally configurable algorithmic pipelines. The \emph{LOC} (Lines Of Code) column reports the number of lines of Python code required for a minimal NSGA-II run on a built-in benchmark problem: \VAMOS{} requires only two lines (Listing~\ref{lst:quickstart}), while other frameworks require importing and assembling multiple components. Finally, the \emph{UI} column indicates whether the framework provides a graphical interface for interactive problem definition and results exploration; only \VAMOS{} offers this through VAMOS Studio.
...
\toprule
\textbf{Framework} & \textbf{Array} & \textbf{Accel.} & \textbf{Config.} & \textbf{LOC\textsuperscript{$\dagger$}} & \textbf{UI} \\
\midrule
\VAMOS{} & Yes & Numba/MooCore & Rich & 2 & Yes \\
pymoo~\cite{blank2020pymoo} & Partial & Moocore & Medium & $\sim$10 & No \\
jMetalPy~\cite{benitez2019jmetalpy} & No & No & Rich & $\sim$15 & No \\
DEAP~\cite{fortin2012deap} & No & No & Low & $\sim$20 & No \\
Platypus~\cite{hadka2015platypus} & No & No & Low & $\sim$8 & No \\
\bottomrule
\multicolumn{6}{l}{\footnotesize\textsuperscript{$\dagger$}Lines of code for a minimal NSGA-II run on a built-in benchmark.}
\end{tabular}
\end{table}
...
Nondominated Sorting Genetic Alg.~III (NSGA-III) & Ref.-point & \cite{deb2014evolutionary} \\
MOEA Based on Decomposition (MOEA/D) & Decomp. & \cite{zhang2007moea} \\
S-Metric Selection EMOA (SMS-EMOA) & Indicator & \cite{beume2007sms} \\
Strength Pareto EA~2 (SPEA2) & Archive & \cite{zitzler2001spea2} \\
Indicator-Based EA (IBEA) & Indicator & \cite{zitzler2004indicator} \\
Speed-Constrained PSO (SMPSO) & Swarm & \cite{nebro2009smpso} \\
Adaptive Geometry Estimation MOEA (AGE-MOEA) & Adaptive & \cite{panichella2019adaptive} \\
Reference Vector Guided EA (RVEA) & Ref.\ vectors & \cite{cheng2016reference} \\
\bottomrule
\end{tabular}
\end{table}
...
\VAMOS{} also provides constraint handling through a declarative interface that supports feasibility rules, penalty functions, and $\varepsilon$-constraint relaxation. Constraints are specified via a lightweight DSL or plain Python callables and integrated into survival selection transparently. Results are returned in a unified result object that supports plotting and post-hoc analysis (e.g., filtering non-dominated solutions and exporting fronts).
...
For Numba-accelerated backends, we distinguish two timing policies. \textbf{Warm} timings exclude one-time JIT compilation by performing a short warmup run in the same process before starting the timer (2{,}000 warmup evaluations; not counted in the reported time). \textbf{Cold} timings measure the first run from a fresh process and therefore include JIT compilation overhead. Unless otherwise stated, we report warm timings to reflect steady-state throughput. Cold-start overhead (JIT compilation on first invocation) adds approximately 1.6--2.3\,s per process across the three representative problems tested (ZDT4, DTLZ2, WFG2), with a median of ${\sim}2$\,s; this cost is amortized on subsequent runs via on-disk caching (\texttt{@njit(cache=True)}).

Experiments were run on a workstation with an Intel(R) Core(TM) Ultra 9 185H central processing unit (CPU) (16 cores, 22 logical processors) and 32\,GB random-access memory (RAM), running Microsoft Windows 11 Home (build 26200).\footnote{All experiments were conducted on Windows~11. Because all frameworks run under the same Python interpreter and OS, the relative framework rankings should generalize across platforms; however, absolute timings may differ on Linux due to differences in process scheduling, memory allocation, and Numba JIT compilation paths.} We used Python 3.12.3 and evaluated \VAMOS{} 0.1.0 (NumPy 2.3.5 with OpenBLAS 0.3.30; Numba 0.63.1) against pymoo 0.6.1.6, jMetalPy (local source checkout at commit \texttt{fecb85c}, accessed January 2026), DEAP 1.4.3, and Platypus (local source checkout at commit \texttt{fe7aeff}, accessed January 2026).
...
\end{tabular}
\end{table}
\footnotetext{Average\textsuperscript{*} in summary tables denotes the mean of the three family-level medians, giving equal weight to each family regardless of its size (ZDT: 5 problems, DTLZ: 7, WFG: 9). Per-problem averages over all 21 problems are reported in the appendix (Tables~\ref{tab:detailed_backends} and~\ref{tab:detailed_comparison}).}
...
For both variants, normalized hypervolume summaries (Tables~\ref{tab:frameworks_hv_nsgaii_ss} and~\ref{tab:frameworks_hv_nsgaii_archive} in Appendix~\ref{app:variants}) confirm that the runtime improvements do not degrade solution quality: family-level HV medians are closely matched across frameworks.
...
\begin{table*}[htbp]
\centering
\caption{NSGA-II. Normalized hypervolume summary (median (IQR)) by problem family across frameworks\protect\footnotemark}
\label{tab:frameworks_hv}
\begin{tabular}{lcccc}
\toprule
\textbf{Framework} & \textbf{ZDT} & \textbf{DTLZ} & \textbf{WFG} & \textbf{Overall\textsuperscript{*}} \\
\midrule
VAMOS & 0.990 (0.008) & 0.799 (0.153) & 0.934 (0.140) & 0.934 (0.181) \\
pymoo & 0.990 (0.008) & 0.792 (0.180) & 0.846 (0.222) & 0.914 (0.192) \\
jMetalPy & 0.990 (0.009) & 0.797 (0.155) & 0.846 (0.224) & 0.915 (0.192) \\
DEAP & 0.990 (0.008) & 0.802 (0.170) & 0.839 (0.226) & 0.917 (0.186) \\
Platypus & 0.990 (0.008) & 0.786 (0.209) & 0.920 (0.139) & 0.920 (0.196) \\
\bottomrule
\end{tabular}
\end{table*}
\footnotetext{No bolding is applied: per-problem Wilcoxon tests (Table~\ref{tab:stats_hypervolume_nsgaii}) show no statistically significant differences after Holm correction (except ZDT6, $\Delta\approx 0.3\%$). Family-level median differences reflect aggregation effects rather than meaningful quality gaps.}

Across ZDT, all frameworks attain nearly identical normalized HV, suggesting that benchmark definitions and operator semantics are effectively aligned. For DTLZ, medians are tightly clustered across frameworks, while WFG exhibits the largest dispersion. We note that in the per-problem NSGA-II comparison (Table~\ref{tab:stats_hypervolume_nsgaii}), pymoo achieves marginally higher median HV on 14 of 21 problems; however, only one difference (ZDT6) is statistically significant after Holm correction, and the paired-bootstrap CIs confirm equivalence ($\pm 1\%$) on 14/21 problems. The family-level aggregates are influenced by WFG9, where \VAMOS{} achieves a substantially higher median (0.934 vs.\ 0.714); excluding this outlier, family-level medians are closely matched. Together with the runtime results, these summaries support improved throughput without degraded final quality under the fixed-reference-front protocol.

To corroborate the HV-based rankings with a convergence-and-diversity indicator, we also report IGD+ (Inverted Generational Distance Plus)~\cite{ishibuchi2015modified}, which measures the proximity of the obtained front to the reference Pareto front without requiring a reference point. IGD+ summary tables are provided in Appendix~\ref{app:igd_tables}. Rankings under IGD+ are broadly concordant with the HV results, confirming that the quality equivalence holds under both indicator families.
...
To verify that the array-based architecture does not alter convergence dynamics, we track normalized hypervolume at 1{,}000-evaluation intervals during NSGA-II runs on three representative problems: ZDT1 (2-objective, convex front), DTLZ2 (3-objective, spherical front), and WFG4 (2-objective, multi-modal front). We compare \VAMOS{} (Numba) against pymoo using 30 independent seeds and the same operator configuration as the main benchmark.

Fig.~\ref{fig:convergence} shows the median HV trajectory with interquartile-range shaded bands for each framework. On all three problems, the convergence curves are closely matched throughout the optimization process, confirming that \VAMOS{}'s vectorized implementation preserves the same search dynamics as the reference framework. The shaded IQR bands overlap substantially, indicating comparable seed-to-seed variability. These results demonstrate that the runtime improvements documented in Section~\ref{sec:experiments} do not come at the cost of slower or altered convergence behavior.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/convergence.png}
\caption{Normalized HV convergence trajectories for NSGA-II: VAMOS (Numba, solid) vs pymoo (dashed). Lines show medians over 30 seeds; shaded bands show IQR. Curves are closely matched, confirming that the vectorized architecture preserves convergence dynamics.}
...
Regarding solution quality, while NSGA-II and SMS-EMOA show statistically equivalent normalized HV across frameworks, the MOEA/D comparison reveals a different pattern: \VAMOS{} achieves significantly higher HV on 11 of 21 problems versus pymoo (Table~\ref{tab:stats_hypervolume_moead}), with a median relative improvement of $+1.4\%$. This divergence is likely attributable to subtle differences in the differential evolution crossover implementation, neighborhood update logic, and weight-vector handling across frameworks---components that are harder to align semantically than the SBX/PM operators used in NSGA-II. Notably, only one problem (WFG9) significantly favors pymoo, and \VAMOS{} achieves equivalent or superior quality on the remaining 20 problems. These results suggest that the vectorized architecture not only preserves but can occasionally improve solution quality, particularly for decomposition-based algorithms where implementation details have a larger impact on search dynamics.
...
Similarly, DTLZ6 achieves relatively low normalized HV (0.408 for NSGA-II across all frameworks) under the standard 50{,}000-evaluation budget. DTLZ6 features a degenerate distance function ($g = \sum x_i^{0.1}$) that creates a difficult landscape; convergence to the true front typically requires budgets of 100{,}000--200{,}000 evaluations. The low HV is consistent across frameworks, confirming it reflects problem--budget interaction rather than framework-specific behavior.
...
The robustness summary (Table~\ref{tab:hv_robustness_summary_nsgaii}) shows that \VAMOS{} achieves the lowest median percentage of near-best seeds (86.7\%) compared to DEAP (100\%), Platypus (96.7\%), and pymoo (93.3\%). This is driven by a small number of WFG problems (notably WFG1 and WFG6) where \VAMOS{} exhibits higher seed-to-seed variability, likely due to subtle differences in SBX boundary handling and duplicate elimination logic. On ZDT and DTLZ problems, \VAMOS{}'s robustness is comparable to other frameworks. We note that the median IQR of normalized HV across problems is similar for all frameworks (0.004--0.006), indicating that the overall dispersion of solution quality is not materially different.
...
%==============================================================================
\appendices
\section{Additional runtime and quality comparisons}
\label{app:variants}
Tables in this appendix report supplementary runtime and solution-quality comparisons for NSGA-II variants (steady-state and external archive), SMS-EMOA, and MOEA/D under the same protocol as Section~\ref{sec:experiments}. Solution-quality tables confirm that the runtime improvements reported in the main text are not obtained at the expense of normalized hypervolume.
...
% Placeholder for NSGA-III and SPEA2 tables (generated by 20_run_additional_algorithms_benchmark.py)
% \input{frameworks_perf_nsgaiii.tex}
% \input{frameworks_perf_spea2.tex}
...
%==============================================================================
\section{IGD+ Quality Tables}
\label{app:igd_tables}
%==============================================================================

Tables in this appendix report IGD+ (Inverted Generational Distance Plus) summaries by problem family, complementing the normalized hypervolume results in the main text. IGD+ measures the mean modified distance from each reference-front point to the nearest obtained point, where the modification clips negative components (i.e., improvements) to zero. Lower values indicate better convergence and diversity. These tables will be generated once benchmark CSVs include the \texttt{igd\_plus} column (see \texttt{paper/16\_update\_igd\_tables\_from\_csv.py}).

% Placeholder: uncomment once IGD+ data is collected
...
\begin{lstlisting}[caption={Defining and optimizing a custom problem.},label={lst:make_problem}]
