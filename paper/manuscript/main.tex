...
\begin{enumerate}[label=\alph*)]
  ...
  \item \textit{Configurability}: Nine MOEA implementations with high component-level configurability and an auto tune tool.
...
Active maintenance                  & \cmark & \pmark & \pmark & \xmark & \cmark & \cmark\\
\bottomrule
...
The core architecture of VAMOS is organized into four layers, as shown in Fig.~\ref{fig:architecture}: a) the \textit{Foundation Layer} providing compute kernels, problem definitions, quality indicators, and \textcolor{red}{archives}; b) the \textit{Engine Layer} implementing multi-objective metaheuristics on top of shared components; c) the \textit{Experiment Layer} exposing CLI tools, benchmarking utilities, visualization and reporting; and d) the \textit{UX Layer} providing VAMOS Studio, a Streamlit-based interactive environment for visual problem definition, live Pareto-front preview, and post-run analysis \textcolor{red}{ (esto no es lo que se ve en la figura)}.
...
\textcolor{red}{Se habla aquí de los kernels, pero no de los problemas, indicadores de calidad, etc., mencionados en la figura 1.}
\subsection{Engine Layer}

VAMOS implements the nine MOEAs \textcolor{red}{ (no todo son MOEAs - SMPSO) } shown in Table~\ref{tab:algorithms}, covering  the three major MOEA paradigms: dominance, decomposition and indicator-based. All nine algorithms are built on top of the shared selection/variation/survival components ensuring consistent operator semantics and enabling component-level substitution without algorithm-specific code changes, so that practitioners can compare paradigms under identical infrastructure and configuration conventions. In addition to the algorithm's selection paradigm, Table~\ref{tab:algorithms} summarizes the supported encodings, number of available crossover and mutation operators, algorithm-specific parameters, and the total number of configurable parameters (base + conditional) in the tuning configuration space. The parameter counts reflect the tuning configuration spaces used by the built-in autotuner (up to 37 for NSGA-II), illustrating the breadth of component-level configurability that VAMOS exposes as first-class configuration option. \textcolor{blue}{  (pero si los 37 de NSGA-II incluyen operadores de todos los encondings, los 37 parámetros no son configurables al mismo tiempo -> cuántos son para optimización continua?. No está SMPSO en la tabla? - no me parece mala idea centrarnos solo en MOEAs) }

\begin{table*}[ht]
\centering
\caption[Overview of the nine MOEAs implemented in VAMOS]{Overview of the nine MOEAs implemented in \VAMOS{} and their configurable parameter spaces across all supported encodings.  R\,=\,real, P\,=\,permutation, B\,=\,binary, I\,=\,integer, M\,=\,mixed}
...
\midrule
NSGA-II & \cite{deb2002fast} & Dominance & R, P, B, I, M & 25 & 24 & Offspring ratio, external archive (type, size, bounded) & 37 \\
...
\midrule
Numba & \textbf{2.66} & \textbf{2.95} & \textbf{3.44} & \textbf{3.02} \\
\bottomrule
\end{tabular}
\end{table}
...
The Numba backend is the fastest across all three families, but the margin varies considerably. For ZDT and DTLZ, Numba and MooCore are nearly equivalent (less than 5\% difference), indicating that the JIT overhead advantage is negligible when objective function evaluations are inexpensive. In contrast, for WFG, where the number of decision variables is larger ($n=24$), Numba is 1.7$\times$ faster than MooCore and 2.6$\times$ faster than NumPy. The pure NumPy backend is consistently the slowest (2.4--2.6$\times$ slower than Numba), confirming that per-generation interpreted Python overhead dominates the cost in the absence of JIT compilation.
...
Table~\ref{tab:frameworks_perf} compares the runtime of generational NSGA-II across five Python frameworks. \VAMOS{} (Numba backend) is the fastest in all three problem families. pymoo is the closest competitor, with runtimes only 1.1--1.2$\times$ higher on ZDT and DTLZ, though the gap widens slightly on WFG. jMetalPy, DEAP, and Platypus are substantially slower (4--10$\times$), reflecting the overhead of per-solution object-oriented representations.
...
\midrule
VAMOS & \textbf{2.62} & \textbf{3.46} & \textbf{4.03} \\
pymoo & 3.22 & 3.86 & 4.56 \\
jMetalPy & 11.55 & 14.91 & 43.94 \\
DEAP & 14.99 & 21.42 & 43.72 \\
Platypus & 20.28 & 29.01 & 50.16 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
NSGA-II (steady-state) & VAMOS & \textbf{8.97} & \textbf{9.98} & \textbf{93.43} \\
 & pymoo & 38.84 & 44.36 & 360.96 \\
 & jMetalPy & 95.51 & 103.32 & 159.09 \\
\midrule
NSGA-II (ext. archive) & VAMOS & \textbf{13.64} & \textbf{39.87} & \textbf{15.52} \\
 & pymoo & 28.92 & 81.54 & 32.35 \\
 & jMetalPy & 18.10 & 44.05 & 46.94 \\
\midrule
SMS-EMOA & VAMOS & \textbf{5.86} & \textbf{6.94} & \textbf{97.84} \\
 & pymoo & 38.35 & 45.81 & 489.74 \\
 & jMetalPy & 92.45 & 100.22 & 170.72 \\
\midrule
MOEA/D & VAMOS & \textbf{3.25} & \textbf{4.05} & \textbf{74.08} \\
 & pymoo & 14.76 & 19.24 & 132.32 \\
 & jMetalPy & 7.63 & 8.17 & 92.70 \\
\bottomrule
\end{tabular}
\end{table}

\VAMOS{} is the fastest framework for all four algorithm configurations across all problem families. The speedup is most pronounced for the steady-state NSGA-II and SMS-EMOA variants, where \VAMOS{} is 4--5$\times$ faster than pymoo on ZDT and DTLZ, and the gap widens further on WFG (3.9$\times$ for steady-state, 5$\times$ for SMS-EMOA). These large margins are expected: both algorithms execute $N$ iterations per generation, amplifying the per-iteration overhead of object-oriented frameworks. For the external archive variant, the advantage is more moderate (about 2$\times$ over pymoo), since the dominant cost shifts to archive maintenance rather than per-iteration dispatch. For MOEA/D, \VAMOS{} is 2--4.5$\times$ faster than pymoo and 1.2--2.3$\times$ faster than jMetalPy, with jMetalPy being notably more competitive on ZDT and DTLZ for this algorithm.
...
\begin{tabular}{lccc}
\toprule
\textbf{Family} & \textbf{VAMOS} & \textbf{pymoo} & \textbf{jMetalPy} \\
\midrule
ZDT & \textbf{0.991 (0.013)} & \textbf{0.991 (0.010)} & \textbf{0.991 (0.014)} \\
DTLZ & \textbf{0.836 (0.121)} & 0.829 (0.148) & 0.835 (0.110) \\
WFG & \textbf{0.934 (0.138)} & 0.846 (0.220) & 0.845 (0.223) \\
\midrule
Overall & \textbf{0.934 (0.157)} & 0.921 (0.156) & 0.924 (0.159) \\
\bottomrule
\end{tabular}
\end{table}
...
Not all comparisons favor \VAMOS{}. For MOEA/D on WFG problems (Table~\ref{tab:frameworks_perf_variants}), jMetalPy achieves lower median runtime (8.34\,s vs.\ 11.12\,s). This is attributable to differences in the decomposition inner loop: jMetalPy's MOEA/D processes one subproblem at a time with lightweight scalar operations, a pattern where Python-level overhead is modest and \VAMOS{}'s array-batching strategy does not yield the same advantage as in generational algorithms with population-wide survival.
...
\multirow{8}{*}{General} & Pop.\ Size $\in \Intdomain{20, 200}$ (log-scale); default $\mathbf{100}$ & -- \\
 ...
 & Selection $\in$ \{\textbf{Tournament}, Boltzmann, Ranking, SUS\} & Pressure $\in \Intdomain{\mathbf{2}, 10}$ \\
...
\multirow{12}{*}{Crossover} & \multicolumn{2}{l}{\textit{Global:} Probability $\in \Realdomain{0.6, 1.0}$, Repair $\in$ \{none, \textbf{clip}, reflect, random, round\}} \\
 ...
 & Arithmetic & -- \\
 & WholeArithmetic & $\alpha \in \Realdomain{0.0, 1.0}$ \\
...
\multirow{10}{*}{Mutation} & \multicolumn{2}{l}{\textit{Global:} Prob.\ Factor $\in \Realdomain{0.25, 3.0}$ ($p_m = \text{factor}/n$), Dist.\ Index $\in \Realdomain{5.0, 40.0}$} \\
 ...
 & UniformReset & -- \\
 & L\'evyFlight & $\beta \in \Realdomain{0.5, 2.0}$, \textit{Scale} $\in \Realdomain{0.001, 0.1}$ \\
 & PowerLaw & \textit{Index} $\in \Realdomain{0.5, 5.0}$ \\ \bottomrule
\end{tabular}%
}
\end{table*}
