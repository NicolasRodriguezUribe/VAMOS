...
\end{enumerate}
The selected racing-tuned configuration is exported for reproducibility (\path{experiments/tuned_nsgaii_resolved.json}) and summarized in Table~\ref{tab:racing_tuned_config}. All variants are evaluated for 30 independent seeds and reported using the same normalized hypervolume protocol (Section~\ref{sec:hv}) and wall-clock runtime measurement setup. Tables~\ref{tab:ablation_runtime} and~\ref{tab:ablation_hypervolume} summarize the ablation outcomes aggregated by benchmark family.
...
\midrule
Baseline & 1.77 & 1.80 & 1.93 & 1.84 \\
Baseline + AOS & 1.87 & 1.85 & 2.02 & 1.91 \\
Racing-tuned & \textbf{1.65} & \textbf{1.71} & \textbf{1.74} & \textbf{1.70} \\
\bottomrule
\end{tabular}
\end{table}
...
Baseline + AOS & 0.991 & 0.885 & 0.877 & 0.917 \\
Racing-tuned & \textbf{0.992} & \textbf{0.935} & 0.870 & \textbf{0.932} \\
\bottomrule
\end{tabular}
\end{table}
