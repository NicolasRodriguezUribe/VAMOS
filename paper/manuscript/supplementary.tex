...
Code~\ref{lst:optuna_tune} shows how to autotune the NSGA-II tuning space with the Optuna backend. The \texttt{build\_nsgaii\_config\_space} function defines the search space, which is converted to a \texttt{ParamSpace} before calling \texttt{ModelBasedTuner}.

\begin{lstlisting}[float=ht!,caption={Autotuning NSGA-II with Optuna on ZDT1, DTLZ2, and WFG4.},label={lst:optuna_tune}]
import numpy as np
