...
def generate_latex_hv_robustness_table(df_rob: pd.DataFrame, *, label: str = "tab:hv_robustness_summary") -> str:
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Robustness summary for normalized hypervolume across seeds (median across problems). ``Near-best'' is defined per problem as HV $\ge 0.99 \cdot \max$ median(HV) among frameworks}",
        rf"\label{{{label}}}",
        r"\begin{tabular}{lcc}",
        r"\toprule",
...
def generate_latex_hv_equivalence_ci_table(df_details: pd.DataFrame, *, label: str = "tab:hv_equivalence_ci") -> str:
    ...
    lines = [
        ...
        r"\tiny",
        r"\centering",
        r"\caption{Per-problem paired bootstrap 90\% confidence intervals (CI) for the relative normalized hypervolume difference $\Delta = (\HV_{\VAMOS} - \HV_{\text{fw}})/\HV_{\text{fw}}$, reported in percent. Equivalence margin is $\pm 1\%$}",
        rf"\label{{{label}}}",
