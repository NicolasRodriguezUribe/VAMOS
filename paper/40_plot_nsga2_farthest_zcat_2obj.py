"""Generate diagnostic figures for the nsga2_farthest 2-objective ZCAT campaign."""
...
def _plot_nsga2_farthest_delta_heatmaps(
    output_dir: Path,
    ...
    metrics = [
        ("hypervolume", "Median HV delta for nsga2_farthest vs comparator"),
        ("igd_plus", "Median IGD+ delta for nsga2_farthest vs comparator"),
    ]
    comparators = [rhs for lhs, rhs in comparisons if lhs == "nsga2_farthest"]
    figure, axes = plt.subplots(1, 2, figsize=(14, 10), constrained_layout=True)

    for axis, (metric, title) in zip(axes, metrics, strict=True):
        matrix = np.zeros((len(problems), len(comparators)), dtype=float)
        for row_idx, problem in enumerate(problems):
            for col_idx, rhs_algorithm in enumerate(comparators):
                row = next(
                    item
                    for item in comparison_rows
                    if item["problem"] == problem
                    and item["metric"] == metric
                    and item["lhs_algorithm"] == "nsga2_farthest"
        ...
        axis.set_xticks(range(len(comparators)), [labels[algorithm] for algorithm in comparators], rotation=20, ha="right")
    ...
    figure.savefig(output_dir / "nsga2_farthest_median_delta_heatmaps.png", dpi=200)
    plt.close(figure)


def _plot_hv_boxplots(output_dir: Path, raw_rows: list[dict[str, str]], problems: list[str], algorithms: list[str], labels: dict[str, str]) -> None:
    grouped: dict[tuple[str, str], list[float]] = defaultdict(list)
    for row in raw_rows:
        grouped[(row["problem"], row["algorithm"])].append(float(row["hypervolume"]))

    figure, axes = plt.subplots(5, 4, figsize=(17, 14), sharey=False, constrained_layout=True)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate diagnostic figures for the nsga2_farthest 2-objective ZCAT campaign."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    labels = {algorithm: run_config.get("algorithm_labels", {}).get(algorithm, ALGORITHM_LABELS.get(algorithm, algorithm)) for algorithm in algorithms}
    comparisons = [tuple(pair) for pair in run_config.get("comparisons", [])]

    _plot_nsga2_farthest_delta_heatmaps(output_dir, comparison_rows, problems, comparisons, labels)
