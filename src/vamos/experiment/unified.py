...
def optimize(
    ...
    *,
    ...
    Args:
        ...
        termination: Optional legacy termination tuple. If provided, it overrides
            the default `("max_evaluations", max_evaluations)` construction.
...
def _run_single(
    ...
    if termination is not None:
        if len(termination) != 2 or not isinstance(termination[0], str):
            raise ConfigurationError("termination must be a tuple of the form ('name', value).")
        termination_name, termination_value = termination
        if termination_name == "max_evaluations":
            termination_budget = _coerce_int("termination[1]", termination_value, min_value=1)
            if max_evaluations is not None and max_evaluations != termination_budget:
                raise ConfigurationError(
                    f"Conflicting max_evaluations values: max_evaluations={max_evaluations} but termination={termination!r}."
                )
            max_evaluations = termination_budget
        elif max_evaluations is not None:
            raise ConfigurationError("max_evaluations cannot be combined with non-'max_evaluations' termination.")
    ...
    if max_evaluations is None and termination is None:
        _auto_defaults["max_evaluations"] = effective_max_evaluations
    effective_termination: tuple[str, object]
    if termination is not None:
    ...
    if verbose:
        ...
        _logger().info(
            "[vamos] Config: %s, pop_size=%s, termination=%s",
            algorithm,
            effective_pop_size,
    ...
    resolved_config = {
        ...
        "max_evaluations": effective_max_evaluations if effective_termination[0] == "max_evaluations" else None,
        "seed": seed,
        "n_var": n_var,
        "n_obj": n_obj,
        "encoding": encoding,
    ...
    max_evaluations_source = "legacy_termination" if termination is not None else ("explicit" if max_evaluations is not None else "auto")
