...
DefaultConfigBuilder = Callable[[int | None, int | None, int | None], AlgorithmConfigProtocol]


def _nsgaii_default(pop_size: int | None, n_var: int | None, _n_obj: int | None) -> AlgorithmConfigProtocol:
    kwargs: dict[str, int] = {}
    if pop_size is not None:
        kwargs["pop_size"] = pop_size
    return NSGAIIConfig.default(n_var=n_var, **kwargs)


def _moead_default(pop_size: int | None, n_var: int | None, n_obj: int | None) -> AlgorithmConfigProtocol:
    n_obj_value = 3 if n_obj is None else n_obj
    return MOEADConfig.default(pop_size=pop_size, n_var=n_var, n_obj=n_obj_value)


def _smsemoa_default(pop_size: int | None, n_var: int | None, _n_obj: int | None) -> AlgorithmConfigProtocol:
    kwargs: dict[str, int] = {}
    if pop_size is not None:
        kwargs["pop_size"] = pop_size
...
def _nsgaiii_default(pop_size: int | None, n_var: int | None, n_obj: int | None) -> AlgorithmConfigProtocol:
    n_obj_value = 3 if n_obj is None else n_obj
    return NSGAIIIConfig.default(pop_size=pop_size, n_var=n_var, n_obj=n_obj_value)


def _spea2_default(pop_size: int | None, n_var: int | None, _n_obj: int | None) -> AlgorithmConfigProtocol:
    kwargs: dict[str, int] = {}
    if pop_size is not None:
        kwargs["pop_size"] = pop_size
...
def _ibea_default(pop_size: int | None, n_var: int | None, _n_obj: int | None) -> AlgorithmConfigProtocol:
    kwargs: dict[str, int] = {}
    if pop_size is not None:
        kwargs["pop_size"] = pop_size
...
def _smpso_default(pop_size: int | None, n_var: int | None, _n_obj: int | None) -> AlgorithmConfigProtocol:
    kwargs: dict[str, int] = {}
    if pop_size is not None:
        kwargs["pop_size"] = pop_size
...
def _agemoea_default(pop_size: int | None, n_var: int | None, _n_obj: int | None) -> AlgorithmConfigProtocol:
    kwargs: dict[str, int] = {}
    if pop_size is not None:
        kwargs["pop_size"] = pop_size
...
def _rvea_default(pop_size: int | None, n_var: int | None, _n_obj: int | None) -> AlgorithmConfigProtocol:
    kwargs: dict[str, int] = {}
    if pop_size is not None:
        kwargs["pop_size"] = pop_size
...
def build_default_algorithm_config(
    ...
    *,
    ...
    if builder is None:
        return None
    return builder(pop_size, n_var, n_obj)
