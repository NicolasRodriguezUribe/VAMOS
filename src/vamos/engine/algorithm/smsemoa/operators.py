...
def _build_binary_operators(
    ...
    if cross_method not in BINARY_CROSSOVER:
        raise ValueError(
            f"Unsupported SMSEMOA crossover '{cross_method}' for binary encoding."
        )
    if mut_method not in BINARY_MUTATION:
        raise ValueError(
            f"Unsupported SMSEMOA mutation '{mut_method}' for binary encoding."
        )
...
def _build_integer_operators(
    ...
    if cross_method not in INT_CROSSOVER:
        raise ValueError(
            f"Unsupported SMSEMOA crossover '{cross_method}' for integer encoding."
        )
    if mut_method not in INT_MUTATION:
        raise ValueError(
            f"Unsupported SMSEMOA mutation '{mut_method}' for integer encoding."
        )
