...
def config_from_assignment(algorithm_name: str, assignment: dict[str, Any]) -> Any:
    ...
    if algo in {"nsgaii", "nsgaii_permutation", "nsgaii_mixed", "nsgaii_binary", "nsgaii_integer"}:
    ...
    if algo in {"moead", "moead_binary", "moead_integer"}:
    ...
    if algo == "moead_permutation":
        ...
        if use_external_archive:
            ...
            builder.archive(archive_size)
        return builder.build()
    if algo in {"nsgaiii", "nsgaiii_binary", "nsgaiii_integer"}:
        ...
        return builder.build()
    if algo in {"smsemoa", "smsemoa_binary", "smsemoa_integer"}:
    ...
    if algo == "spea2":
        ...
        return builder.build()
    if algo in {"ibea", "ibea_binary", "ibea_integer"}:
